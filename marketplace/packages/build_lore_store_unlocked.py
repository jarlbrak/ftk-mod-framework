#!/usr/bin/env python3
"""Build deterministic Lore Store Unlocked release candidates without publishing."""
import argparse
import hashlib
import io
import json
...
PACKAGE = Path(__file__).resolve().parent / 'lore-store-unlocked'
...
def main():
    ...
    files = {}
    paths = [PACKAGE / 'manifest.json', PACKAGE / 'content.json']
    for p in paths:
    ...
    sha = digest(payload)
    stem = 'lore-store-unlocked-' + manifest['version'] + '-' + sha[:12]
    base = 'https://github.com/jarlbrak/ftk-mod-framework/releases/download/lore-store-unlocked-v' + manifest['version'] + '/'
    ...
    descriptor.update(frameworkRange='>=' + manifest['frameworkVersion'] + ' <2.0.0',
                      ...
                      fileCount=len(files), screenshots=[base + 'lore-store-unlocked-banner.jpg'])
