"""Create editable Bramblecoil scenes, re-export the source, and render studio views."""
...
REFERENCE = ROOT / "scratch/skeleton-audit/121424"
SCRATCH = ROOT / "scratch/bramblecoil-jungle-snake-roundtrip"
...
def import_original(name: str, armature):
    ...
    uv = mesh.uv_layers.new(name="Bramblecoil palette")
    ...
    obj["ftk_export"] = True
    image = bpy.data.images.load(str(OUT / "bramblecoil-jungle-snake_basecolor.png"), check_existing=True)
    image.pack()
    material = bpy.data.materials.new("Bramblecoil original palette")
...
create(REFERENCE / "reference.npz", REFERENCE / "skeleton.json", SCRATCH / "bramblecoil-template.blend", PYTHON, False)
armature = next(obj for obj in bpy.context.scene.objects if obj.type == "ARMATURE")
for obj in list(bpy.context.scene.objects):
    if obj.type == "MESH":
        bpy.data.objects.remove(obj, do_unlink=True)
model = import_original("bramblecoil-jungle-snake", armature)
bpy.context.scene["ftk_art_status"] = "Original Bramblecoil Viper source; live validation pending."
bpy.context.view_layer.objects.active = model
model.select_set(True)
armature.select_set(False)
bpy.ops.wm.save_as_mainfile(filepath=str(OUT / "bramblecoil-jungle-snake.blend"))
bpy.ops.wm.open_mainfile(filepath=str(OUT / "bramblecoil-jungle-snake.blend"))
export(OUT / "bramblecoil-jungle-snake-reopened.glb")
result = subprocess.run(
    [
        str(PYTHON), str(ROOT / "tools/ai-model-pipeline/validate_glb.py"),
        str(OUT / "bramblecoil-jungle-snake-reopened.glb"),
...
scene["ftk_art_status"] = "Studio presentation only; export from bramblecoil-jungle-snake.blend."
world = bpy.data.worlds.new("Rainforest studio")
world.use_nodes = True
world.node_tree.nodes["Background"].inputs[0].default_value = (0.005, 0.018, 0.012, 1)
...
floor_material = bpy.data.materials.new("Rainforest floor")
floor_material.use_nodes = True
floor_material.node_tree.nodes["Principled BSDF"].inputs["Base Color"].default_value = (0.008, 0.027, 0.018, 1)
...
light("Sunlit canopy key", (8, -9, 11), 2800, (0.62, 1.0, 0.72), 6)
light("Orchid fill", (-10, -8, 6), 1700, (0.88, 0.34, 0.48), 5)
light("Amber rim", (0, 13, 10), 2600, (1.0, 0.64, 0.23), 6)
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT / "bramblecoil-jungle-snake-studio.blend"))
