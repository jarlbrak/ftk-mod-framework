"""Create editable armature scenes, independently re-export, and render original Tamarind Trickster."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/tamarind-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'tamarind_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Tamarind Trickster original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121301,'tamarind')]:
 ref=ROOT/'scratch/skeleton-audit/121301';create(ref/'reference.npz',ref/'skeleton.json',SCRATCH/f'{name}-template.blend',PYTHON,False)
 arm=next(o for o in bpy.data.objects if o.type=='ARMATURE')
 for obj in list(bpy.data.objects):
  if obj.type=='MESH':bpy.data.objects.remove(obj,do_unlink=True)
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Tamarind Trickster surface; live validation pending'
 ...
 bpy.ops.wm.open_mainfile(filepath=str(OUT/f'{name}.blend'));export(OUT/'tamarind-reopened.glb')
 validation=subprocess.run([str(PYTHON),str(ROOT/'tools/ai-model-pipeline/validate_glb.py'),str(OUT/'tamarind-reopened.glb'),'--reference',str(ref/'reference.npz')],capture_output=True,text=True,check=True)
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'tamarind.blend'))
for obj in bpy.context.scene.objects:
 if obj.type=='ARMATURE':obj.hide_render=True;obj.hide_set(True)
scene=bpy.context.scene;scene.render.fps=12;scene['ftk_art_status']='Studio presentation: export original from tamarind.blend'
...
cam.location=(3,-7,3);aim(cam,(0,0,.72));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-7,-1,3.2);aim(cam,(0,0,.72));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(3,-7,3);aim(cam,(0,0,.72));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'tamarind-studio.blend'))
