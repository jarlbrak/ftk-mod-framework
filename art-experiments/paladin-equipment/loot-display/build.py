#!/usr/bin/env python3
"""Fit original two-handed loot meshes into the live-verified original 1H envelope."""
...
def build(output):
 ...
 one=matrix(next(v for v in binding if v['path']=='smithhammer'))
 two=matrix(next(v for v in binding if v['path']=='hammer'))
 ...
 records=[];files={}
 for tier in SETS:
  key='paladin-hammer-2h-'+tier+'-display';original=OUT.parent/('paladin-hammer-2h-'+tier+'.source.json')
  ...
  # X/Y determine the visible card envelope. A 5% margin stays inside the verified 1H card.
 ...
 report={'schema':'ftkmf.paladin-original-hammer-display.v1','generatorSha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'bindingMetadataSha256':hashlib.sha256(BINDING.read_bytes()).hexdigest(),'referenceEnvelope':reference.relative_to(ROOT).as_posix(),'referenceEnvelopeSha256':hashlib.sha256(reference.read_bytes()).hexdigest(),'referenceOriginalSourceSha256':envelope['referenceOriginalSourceSha256'],'referenceRootMin':lo.tolist(),'referenceRootMax':hi.tolist(),'provenance':'Existing original 2H geometry transformed only by uniform scale and translation in native display-root space. Native data contributes attachment transforms only. Target is the frozen previously authored novice1H envelope, whose card fit was observed live; no native surface geometry used.','limits':'Offline containment in the verified original1H envelope. New2H native card fit remains pending; equipped meshes unchanged.','assets':records,'files':files}
 (output/'manifest.json').write_text(json.dumps(report,indent=2)+'\n')
 print('PASS: six original2H display GLBs fit the original1H reference envelope offline')
