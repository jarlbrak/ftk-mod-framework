#!/usr/bin/env python3
"""Append frozen Tidecrown to one fresh isolated test-catalog stage."""
import argparse
import copy
import hashlib
import json
...
ART = REPO / 'art-experiments/tidecrown-sea-king'
...
probe = next(row for row in old['profiles'] if row['key'] == 'ftkmf_modeltest_probe_seaking')
assert probe['baseEnemy'] == profile['baseEnemy'] == 'seaKing'
assert probe['combatProfile'] == profile['combatProfile']
assert probe['renderers'][0]['rendererPath'] == profile['renderers'][0]['rendererPath'] == 'enSeaKing'
