...
internal static class EnemyLifetimePolicy
{
    ...
    static JObject Descriptor(JObject r)
    {
        ...
        string kind=RendererKind(r);JObject result=new JObject{{"rendererPath",r["rendererPath"].DeepClone()},{"glbFile",r["glbFile"].DeepClone()},{"textureFile",r["textureFile"]==null?new JValue((object)null):r["textureFile"].DeepClone()},{"disableNativeEmission",r["disableNativeEmission"]==null?false:B(r,"disableNativeEmission")},{"rendererKind",kind}};
    ...
    static string RendererKind(JObject renderer)
    {
        JToken token=renderer==null?null:renderer["rendererKind"];string kind=token==null?"SkinnedMeshRenderer":token.Type==JTokenType.String?(string)token:null;
        if(kind!="SkinnedMeshRenderer"&&kind!="MeshRenderer")throw new InvalidOperationException("Invalid lifetime renderer kind.");
        return kind;
    }
    ...
    internal static void Registration(string expected,JObject actual,JObject profile)
    {
        ...
        if(expected!="legacy-singular"){JArray descriptors=new JArray(),actualDescriptors=new JArray();foreach(JObject r in (JArray)profile["renderers"])descriptors.Add(Descriptor(r));foreach(JObject r in (JArray)actual["renderers"])actualDescriptors.Add(Descriptor(r));if(!JToken.DeepEquals(descriptors,actualDescriptors))throw new InvalidOperationException("Actual plural descriptors differ from profile.");return;}
    ...
    internal static void Compare(JObject record)
    {
        ...
        for(int i=0;i<sr.Count;i++)
        {
            ...
            string kind=RendererKind(s);
            if((string)s["rendererPath"]!=(string)c["rendererPath"]||I(s,"rendererId")==I(c,"rendererId")||I(s,"meshId")!=I(c,"meshId")||
                kind!=RendererKind(c))throw new InvalidOperationException("Native clone GLB/renderer mapping differs from source.");
            if(kind=="SkinnedMeshRenderer")
            {
                if((string)s["boneSignature"]!=(string)c["boneSignature"])throw new InvalidOperationException("Native clone GLB/rig mapping differs from source.");
            ...
            else if(I(s,"meshFilterId")==0||I(c,"meshFilterId")==0||I(s,"meshFilterId")==I(c,"meshFilterId"))
                throw new InvalidOperationException("Native static MeshFilter mapping differs from source.");
