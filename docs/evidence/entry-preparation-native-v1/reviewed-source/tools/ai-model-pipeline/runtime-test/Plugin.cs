...
public sealed partial class RuntimeModelTest : BaseUnityPlugin
{
    ...
    void Update()
    {
        ...
        try
        {
            ...
            if (op == "inventory") Finish(id, Inventory(command));
    ...
    IEnumerator Capture(string id,SkinnedMeshRenderer renderer,float seconds,float fps,int maxWidth,bool fixedStep,string provenance,bool materialObservation=false)
    {
        ...
        try
        {
            ...
            JArray frames=new JArray();float started=Time.realtimeSinceStartup,gameStarted=Time.time,unscaledStarted=Time.unscaledTime;
    ...
    void OnDestroy(){if(entryTicket!=null)entryTicket.valid=false;krakenSkinArm=null;portraitArmed=false;portraitTrace.Clear();if(portraitObserver==this)portraitObserver=null;RestoreTutorials();watchedLeases.Clear();}
