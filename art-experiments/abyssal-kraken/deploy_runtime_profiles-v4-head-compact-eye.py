#!/usr/bin/env python3
"""Review and, only with --execute, deploy the V4 Kraken head compact-eye stage."""
...
assert receipt["status"] == "STAGED_NOT_DEPLOYED_ABYSSAL_KRAKEN_V4_HEAD_COMPACT_EYE"
...
assert_game_stopped(GAME)
backup = GAME / "deployment-backups" / ("abyssal-kraken-v4-head-compact-eye-" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
