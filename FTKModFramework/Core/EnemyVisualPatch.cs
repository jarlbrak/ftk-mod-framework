        ...
        private static void ApplyMeshSwap(string enemyId, CharacterEventListener cel, EnemyVisual v)
        {
            ...
            if (!string.IsNullOrEmpty(v.glbMesh))
            {
                ...
                if (gmesh != null)
                {
                    ...
                    // After the FR-2 rebind, the boss renders as a coherent body PLUS a separate floating cloud of
                    // boss-textured triangles. A single rigid mesh on a single SMR cannot split, so there must be a
                    // SECOND renderer (or mesh / SMR) drawing the boss geometry. Enumerate EVERY renderer under the
                    // CEL's whole hierarchy (root, inactive included) and log one line each so we can find the
                    // duplicate. Logging only: no behavior change, never throws.
                    try
                    {
                        const string DiagTag = "[enemy-visual][TEMP-DIAG #74]";
                        Transform diagRoot = (cel != null && cel.transform != null && cel.transform.root != null)
                            ? cel.transform.root
                            : (cel != null ? cel.transform : null);
                        if (diagRoot != null)
                        {
                            Plugin.Log.LogInfo(DiagTag + " === renderer dump for '" + enemyId + "' (root '" +
                                diagRoot.name + "', path " + FullPath(diagRoot) + ") ===");

                            Renderer[] diagRends = diagRoot.GetComponentsInChildren<Renderer>(true);
                            int total = 0;
                            int customMeshCount = 0;
                            if (diagRends != null)
                            {
                                for (int di = 0; di < diagRends.Length; di++)
                                {
                                    Renderer dr = diagRends[di];
                                    if (dr == null) continue;
                                    total++;

                                    StringBuilder sb = new StringBuilder();
                                    sb.Append(DiagTag).Append(' ');
                                    Transform dt = dr.transform;
                                    sb.Append("path=").Append(dt != null ? FullPath(dt) : "(null)");
                                    sb.Append(" type=").Append(dr.GetType().Name);
                                    sb.Append(" enabled=").Append(dr.enabled);
                                    GameObject dgo = dr.gameObject;
                                    sb.Append(" activeInHierarchy=").Append(dgo != null ? dgo.activeInHierarchy : false);

                                    SkinnedMeshRenderer dsmr = dr as SkinnedMeshRenderer;
                                    if (dsmr != null)
                                    {
                                        Mesh sm = dsmr.sharedMesh;
                                        sb.Append(" sharedMesh=").Append(sm != null ? sm.name : "(null)");
                                        sb.Append(" verts=").Append(sm != null ? sm.vertexCount : -1);
                                        sb.Append(" bones=").Append(dsmr.bones != null ? dsmr.bones.Length : -1);
                                        int bindCount = -1;
                                        if (sm != null && sm.bindposes != null) bindCount = sm.bindposes.Length;
                                        sb.Append(" bindposes=").Append(bindCount);
                                        sb.Append(" rootBone=").Append(
                                            dsmr.rootBone != null ? dsmr.rootBone.name : "(null)");
                                        if (sm != null && sm.name != null &&
                                            sm.name.StartsWith("ftkmf_glb_", StringComparison.Ordinal))
                                            customMeshCount++;
                                    }

                                    MeshRenderer dmr = dr as MeshRenderer;
                                    if (dmr != null)
                                    {
                                        MeshFilter dmf = dr.GetComponent<MeshFilter>();
                                        Mesh fm = (dmf != null) ? dmf.sharedMesh : null;
                                        sb.Append(" filterMesh=").Append(fm != null ? fm.name : "(null)");
                                        sb.Append(" verts=").Append(fm != null ? fm.vertexCount : -1);
                                        if (fm != null && fm.name != null &&
                                            fm.name.StartsWith("ftkmf_glb_", StringComparison.Ordinal))
                                            customMeshCount++;
                                    }

                                    Material[] dmats = dr.sharedMaterials;
                                    int matCount = (dmats != null) ? dmats.Length : 0;
                                    sb.Append(" mats=").Append(matCount);
                                    string firstTex = "(none)";
                                    if (dmats != null && dmats.Length > 0 && dmats[0] != null)
                                    {
                                        Texture mt = dmats[0].mainTexture;
                                        firstTex = (mt != null) ? mt.name : "(none)";
                                    }
                                    sb.Append(" tex0=").Append(firstTex);

                                    Plugin.Log.LogInfo(sb.ToString());
                                }
                            }

                            Plugin.Log.LogInfo(DiagTag + " SUMMARY for '" + enemyId + "': totalRenderers=" + total +
                                ", customMesh(ftkmf_glb_*)=" + customMeshCount + ".");
                        }
                        else
                        {
                            Plugin.Log.LogWarning(DiagTag + " could not resolve CEL root for '" + enemyId +
                                "'; renderer dump skipped.");
                        }
                    }
                    catch (Exception de)
                    {
                        Plugin.Log.LogWarning("[enemy-visual][TEMP-DIAG #74] renderer dump failed for '" + enemyId +
                            "': " + de.Message);
                    }
