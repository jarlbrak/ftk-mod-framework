...
public sealed partial class RuntimeModelTest
{
    ...
    static void PortraitRenderPrefix(OffscreenCamera __instance)
    {
        ...
        try
        {
            ...
            if(s.identity=="verified_live_enemy_dummy")
            {
                EnemyDummy dummy=s.source==null?null:s.source.m_Dummy as EnemyDummy;
                if(dummy==null||dummy.m_EventListener!=s.source)throw new InvalidOperationException("Verified live portrait identity lost before render.");
                record["enemyDummyInstanceId"]=dummy.GetInstanceID();
                record["fid"]=new JObject{{"photonId",dummy.FID.m_PhotonID},{"turnIndex",dummy.FID.m_TurnIndex}};
            }
