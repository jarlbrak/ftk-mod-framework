#!/usr/bin/env python3
"""Create editable Blender source and studio views for the compact V5 rigid Kraken eye companion."""
...
def make_source():
    ...
    obj["ftk_art_status"] = "Original compact V5 static companion. Export through build_static_eye.py."
    uv = mesh.uv_layers.new(name="Abyssal V5 eye original UV")
    ...
    image.pack()
    material = bpy.data.materials.new("Abyssal V5 eye original palette")
...
def render_studio(obj):
    ...
    world = bpy.data.worlds.new("Abyssal V5 eye studio")
    ...
    floor_material = bpy.data.materials.new("Abyssal V5 eye studio floor")
    ...
    # The source is deliberately compact. Scale the studio lighting with the
    # object span so close area lights cannot wash out its small silhouette.
    studio_energy = (span / 4.0) ** 2
    add_area("Sea-glass key", center + Vector((span * 1.25, -span * 1.6, span * 1.4)), 8500 * studio_energy, (.18, .93, .78), span * .7, center)
    add_area("Antique-gold fill", center + Vector((-span * 1.45, -span * .7, span * .75)), 5600 * studio_energy, (1.0, .52, .17), span * .58, center)
    add_area("Violet rim", center + Vector((0, span * 1.65, span * 1.15)), 7000 * studio_energy, (.48, .20, .95), span * .7, center)
...
def main():
    ...
    # Start from an empty source scene. The factory Cube in a new Blender file
    # can fully occlude a compact authoring asset in its studio renders.
    ...
    VALIDATION.write_text(json.dumps({
        "status": "PASS_ABYSSAL_KRAKEN_STATIC_EYE_V5_BLEND_REOPEN",
        "asset": ASSET,
    ...
    print(json.dumps({"status": "PASS_ABYSSAL_KRAKEN_STATIC_EYE_V5_BLEND_REOPEN", "asset": ASSET}, indent=2))
