...
COLORS = ['29323B', '394652', '465A64', '527478', '624A38', '7D6047',
          '34302D', '9B8153', '727B85', '8999A0', 'B8C1BE', '65434A']
...
def crown():
    s = m.rigid()
    # The known Street crown envelope clears the scalp. The cloth droop grows
    # only toward the left; lowering the complete crown re-exposes native skin.
    specs=[(-.01,.252,-.105,.226,.212),(-.018,.325,-.10,.263,.258),
           (-.016,.414,-.10,.226,.250),(-.019,.461,-.105,.170,.195),
           (-.045,.510,-.11,.085,.105)]
    s.tube('Soft slouched cloth crown',[p(x,y,z) for x,y,z,rx,rz in specs],
           [v[3] for v in specs],[v[4] for v in specs],['Root']*5,1,sides=10)
    # This broad fold follows the OUTSIDE of the crown, not a flat crossing
    # ribbon; equal cross-section samples keep it attached around both temples.
    rows=[]
    for t in np.linspace(0,math.pi,7):
        upper=p(-.01+.275*math.cos(t),.324-.022*math.cos(t),-.10+.272*math.sin(t))
        lower=p(-.01+.239*math.cos(t),.263-.010*math.cos(t),-.105+.228*math.sin(t))
        rows.append((upper,lower,'Root'))
    ribbon(s,'Single broad brow fold',rows,0,.010)
    return s
...
def coat(s, male):
    ...
    s.tube('Continuous tailored jerkin body', [B[n] for n in spine],
           [w*.84,w*.81,w*.94,w], [.124,.128,.139,.148], spine, 2, sides=10)
    s.tube('Upper cloth shoulder yoke', [B['Chest_M'], B['Chest_M']+p(y=.18), B['Neck_M']-p(y=.027)],
           [w,w*.94,.088], [.147,.124,.083], ['Chest_M','Chest_M','Neck_M'], 2, sides=10)
    s.tube('Continuous trouser pelvis', [B['Root_M']-p(y=.13), B['Root_M']+p(y=.08)],
           [w*.66,w*.95], [.11,.126], ['Root_M']*2, 0, sides=10)
    # Narrow folded seam across a continuous curved jerkin. Cloth panels
    # share one material so the front reads as a garment, not a pasted triangle.
    rows=[]
    for bone,x,y,z in [('Root_M',w*.69,.055,.169),('BackA_M',w*.63,0,.172),
                      ('BackB_M',w*.29,0,.202),('Chest_M',-w*.12,.125,.184),
                      ('Chest_M',-w*.41,.223,.144)]:
        q=B[bone]+p(x,y,z)
        rows.append((q+p(-.013,.006,.005),q+p(.013,-.006,.005),bone))
    ribbon(s,'Narrow diagonal folded closure',rows,0,.014)
    for side, sign in [('R',1),('L',-1)]:
        ...
        ribbon(s, 'Short split hem '+side, [
            ...
            (B['Root_M']+p(sign*.036,-.130,.155), B['Root_M']+p(sign*w*1.11,-.110,.074),{hip:.35,'Root_M':.65})], 2)
        ribbon(s, 'Continuous side hem '+side, [
            (B['Root_M']+p(sign*w*.90,.052,.114), B['Root_M']+p(sign*w*.88,.052,-.108),'Root_M'),
            (B['Root_M']+p(sign*w*1.10,-.110,.074), B['Root_M']+p(sign*w*1.04,-.124,-.098),{hip:.35,'Root_M':.65})],2)
        ...
        s.tube('Tapered one-piece leather bracer '+side, [start,B[el]*.32+B[wr]*.68,end],
               [.098,.090,.075],[.096,.088,.074],[{el:.75,wr:.25},{el:.30,wr:.70},wr],4,sides=8,axis=(0,0,1))
        q=B[el]*.51+B[wr]*.49
        s.tube('Single bracer retaining strap '+side, [q-p(x=sign*.020),q+p(x=sign*.020)],
               [.101]*2,[.099]*2,[{el:.55,wr:.45}]*2,6,sides=8,axis=(0,0,1))
    ...
    s.tube('Soft low collar',[B['Neck_M']-p(y=.074),B['Neck_M']-p(y=.025)],
           [.107,.101],[.096,.088],['Neck_M']*2,2,sides=10)
    if 'Head_M' in B:
...
def boots(s):
    ...
    for side in ['R','L']:
        ...
        s.tube('Same-leather rolled boot lip '+side,[B[ankle]+p(y=.187),B[ankle]+p(y=.204)],
               [.102,.100],[.092,.090],[blend]*2,4,sides=8)
        floor=B[toe][1]-.035
...
def dagger():
    ...
    outline=[(-.043,.119),(.043,.119),(.054,.338),(-.008,.515),(-.045,.350)]
    inner=[(-.037,.123),(.035,.123),(.045,.336),(-.008,.493),(-.039,.347)]
    n=len(outline)
...
def main():
    ...
    save('manifest.json',{'schema':'ftkmf.thief-street-art-prototype.v2','revision':2,'generator':'build.py',
