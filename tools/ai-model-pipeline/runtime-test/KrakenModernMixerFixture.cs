...
public sealed partial class RuntimeModelTest
{
    ...
    sealed class KrakenModernInputMixer
    {
        ...
        public const string Mode="two-clip-raw-v1",FixedMode="fixed-four-raw-v1",BankMode="fixed-two-bank-five-raw-v1";
        static readonly string[] FixedClips={"krakenIdle","krakenDamage","krakenDisappear","kraken_appear"};
        static readonly string[] BankClips={"krakenIdle","krakenDamage","krakenDisappear","kraken_appear","krakenAttack"};
        string selectedMode=Mode;bool Bank{get{return selectedMode==BankMode;}}bool Fixed{get{return selectedMode==FixedMode || Bank;}}
        string SlotName(int i){return Bank?BankClips[i%5]:FixedClips[i];}
        int[] nodeHashes;int mixerHash;
        ...
        string ExpectedClip(int hash)
        {
            if(Bank && hash==Animator.StringToHash("Base Layer.INTRO"))return "krakenIdle";
        ...
        {
            if(requestedMode!=Mode && requestedMode!=FixedMode && requestedMode!=BankMode)throw new ArgumentException("Exact mixer mode required.");selectedMode=requestedMode;inputs=new AnimationClipPlayable[Bank?10:Fixed?4:2];
            ReadTree(source.transform,"",sourceNodes);sourceHash=HashTree(sourceNodes);
            foreach(AnimationClip clip in native.animationClips)
                if(clip!=null && (clip.name=="krakenIdle" || clip.name=="krakenDamage" || clip.name=="krakenDisappear" || clip.name=="kraken_appear" || (Bank && clip.name=="krakenAttack")))
            ...
            if(clips.Count!=(Bank?5:4))throw new InvalidOperationException("Four exact native mixer clip identities required.");
            ...
            if(Fixed){for(int i=0;i<inputs.Length;i++){Replace(i,clips[SlotName(i)]);mixer.SetInputWeight(i,i==0?1:0);inputs[i].SetTime(0);}}
            ...
            if(Bank){nodeHashes=new int[inputs.Length];HashSet<int> hashes=new HashSet<int>();mixerHash=mixer.GetHandle().GetHashCode();hashes.Add(mixerHash);for(int i=0;i<inputs.Length;i++){nodeHashes[i]=inputs[i].GetHandle().GetHashCode();if(!hashes.Add(nodeHashes[i]))throw new InvalidOperationException("Ambiguous owned playable handle hash.");}}
            ...
            if(Fixed){identity["fixedClipOrder"]=Bank?new JArray("krakenIdle","krakenDamage","krakenDisappear","kraken_appear","krakenAttack","krakenIdle","krakenDamage","krakenDisappear","kraken_appear","krakenAttack"):new JArray(FixedClips);identity["fixedClipInstanceIds"]=ClipIds();identity["bindingInitialization"]=Bank?"Both five-clip banks connected before Animator.Rebind; fixed topology throughout run.":"All four clip nodes connected before Animator.Rebind; fixed topology throughout run.";if(Bank){identity["nodeHandleHashes"]=new JArray(nodeHashes);identity["mixerHandleHash"]=mixerHash;identity["slotRoles"]=new JArray("current","current","current","current","current","next","next","next","next","next");}}
        }
        void Audit()
        {
            ...
            if(Bank && mixer.GetHandle().GetHashCode()!=mixerHash)throw new InvalidOperationException("Owned mixer handle changed.");
            for(int i=0;i<inputs.Length;i++)if((Bank && inputs[i].GetHandle().GetHashCode()!=nodeHashes[i]) || !inputs[i].IsValid() || inputs[i].GetApplyFootIK() || inputs[i].GetSpeed()!=0 || !mixer.GetInput(i).GetHandle().Equals(inputs[i].GetHandle()) || (Fixed && inputs[i].GetAnimationClip()!=clips[SlotName(i)]))throw new InvalidOperationException("Mixer input graph changed.");
        }
        JObject Evaluate(Input first,Input next)
        {
            ...
            if(Fixed)
            {
                if(!Bank && next!=null && next.clip==first.clip)throw new InvalidOperationException("Fixed-four mode cannot represent two active clocks for the same native clip.");
                for(int i=0;i<inputs.Length;i++){inputs[i].SetTime(0);mixer.SetInputWeight(i,0);}
                int slot=Array.IndexOf(Bank?BankClips:FixedClips,first.clip.name);inputs[slot].SetTime(first.seconds);mixer.SetInputWeight(slot,first.weight);
                if(next!=null){slot=(Bank?5:0)+Array.IndexOf(Bank?BankClips:FixedClips,next.clip.name);inputs[slot].SetTime(next.seconds);mixer.SetInputWeight(slot,next.weight);}
            }
            else
            {
        ...
        public JObject Step(KrakenControllerSurface modern,KrakenControllerSurface old,JObject frozenNative)
        {
            ...
            if(Fixed)result["fixedClipInstanceIds"]=ClipIds();if(Bank){result["nodeHandleHashes"]=new JArray(nodeHashes);result["mixerHandleHash"]=mixerHash;}return result;
        }
