...
boss ("Mudwretch Foreman", a cloned enemy with signature procs), and a true-victory questline. The boss is
the **culmination of the Flooded Crypt**, not an overworld set-piece: `RealmProperties.m_BossEnemy="None"`
(no freely killable overworld POI, which playtesting found did not fire victory), and the per-id dungeon
override `m_DungeonDefOverride["FloodedCrypt"]` sets its final room to `m_BossType="Specific"` with the
custom set, so the boss is the crypt's last fight. The last quest of the last stage is the Clear-Dungeon
quest on that crypt (`m_EndGameAfterLastQuest = true`), so clearing the crypt = beating the Foreman =
victory. Load-time self-tests (`SELF-TEST PASS [realm-boss-set]` and `SELF-TEST PASS [realm-boss]`) confirm
...
"The Hollow Mire" run advances to the clear-crypt `DungeonQuestLogic` (the last quest), enters and clears
the Flooded Crypt (its POI deactivates, which is exactly the `DungeonQuestLogic.IsCompleteState`
condition), and the last-quest completion fires the victory FSM and the end-game CreditScreen
("CONGRATULATIONS!", honoring `m_EndGameAfterLastQuest`). The harness also starts real in-dungeon combat
(`MiniHexDungeon.Encounter` with `EncounterLocation.Dungeon`); enemy-first turns currently stall on the
dungeon camera FSM under the harness's synthetic entry, a test-tooling limitation that does not affect the
crypt-clear-to-victory path.
