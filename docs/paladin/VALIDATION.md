...
save/resume, online co-op, and Windows/Linux gameplay remain open. [Spec
...
The [rebuilt-binary world-start receipt](current-package-world-start-rebuilt.json)
refreshes the runtime identities from a clean framework and helper build. None
of these receipts accepts motion, ordinary acquisition, other appearances, online
co-op or public publication. The user separately completed the current isolated
package Save and Exit flow and confirmed a fresh resume with the Paladin starter
equipment retained. This is user-operated live validation, not an automation receipt.
...
| Enabled save/resume | Same [production receipt](production-new-game-observation.json): matching starting equipment after same-process save/resume and application restart. The user also manually verified the current `06a6356760be8b167948751013efa497ebdeb3a9e5bfd33417f68b35f329c3db` candidate through its native Save and Exit flow followed by a fresh resume with Paladin starter equipment retained. [Collected loot receipt](custom-loot-collection.json): collected count retained after same-process resume | User-operated current-candidate validation has no automation receipt. Disabled-package recovery, every appearance and full-campaign saves are not established |
...
| Focused-healing rule sequences | GuardianCombat passes 145 checks, including Mercy +2/+4 percent after Guard expiry, miss/dodge followed by absorbed and damaging splash hits, three Focus without multiplication, capped healing and independent guardian receipts. Installed-method metadata checks: 21 pass | Pure state and hit-classification checks do not execute native QueueHealing/OnImpact callbacks or prove live AoE, equipment lookup or networking |
...
06a6 archive and helper identities. The verifier checks exact archive bytes and
