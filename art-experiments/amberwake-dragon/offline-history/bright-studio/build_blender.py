"""Create editable armature scenes, independently re-export, and render original Amberwake Dragon."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/amberwake-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'amberwake_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Amberwake Dragon original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121525,'amberwake')]:
 ref=ROOT/'scratch/skeleton-audit/121525';create(ref/'reference.npz',ref/'skeleton.json',SCRATCH/f'{name}-template.blend',PYTHON,False)
 arm=next(o for o in bpy.data.objects if o.type=='ARMATURE')
 for obj in list(bpy.data.objects):
  if obj.type=='MESH':bpy.data.objects.remove(obj,do_unlink=True)
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Amberwake Dragon surface; live validation pending'
 ...
 bpy.ops.wm.open_mainfile(filepath=str(OUT/f'{name}.blend'));export(OUT/'amberwake-reopened.glb')
 validation=subprocess.run([str(PYTHON),str(ROOT/'tools/ai-model-pipeline/validate_glb.py'),str(OUT/'amberwake-reopened.glb'),'--reference',str(ref/'reference.npz')],capture_output=True,text=True,check=True)
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'amberwake.blend'))
for obj in bpy.context.scene.objects:
 if obj.type=='ARMATURE':obj.hide_render=True;obj.hide_set(True)
scene=bpy.context.scene;scene.render.fps=12;scene['ftk_art_status']='Studio presentation: export original from amberwake.blend'
...
def light(name,loc,power,color,size):
 data=bpy.data.lights.new(name,'AREA');data.energy=power;data.color=color;data.shape='DISK';data.size=size;obj=bpy.data.objects.new(name,data);scene.collection.objects.link(obj);obj.location=loc;aim(obj,(0,2,2))
light('Warm key',(18,-20,26),45000,(.98,.90,.80),15);light('Cool fill',(-18,-8,20),30000,(.59,.78,1),12);light('Copper rim',(0,20,24),60000,(1,.65,.35),10)
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=29.0
...
cam.location=(18,-28,24);aim(cam,(0,2,2));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-28,-3,15);aim(cam,(0,2,2));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(18,-28,24);aim(cam,(0,2,2));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'amberwake-studio.blend'))
