...
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'mournglass_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Mournglass Wraith original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
...
for rid,name in [(121008,'mournglass')]:
 ...
 bpy.ops.wm.open_mainfile(filepath=str(OUT/f'{name}.blend'));export(OUT/'mournglass-reopened.glb')
