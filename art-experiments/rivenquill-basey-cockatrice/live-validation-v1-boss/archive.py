#!/usr/bin/env python3
"""Create the immutable Rivenquill Cockatrice boss-route evidence archive.
...
OUT = ASSET / "live-validation-v1-boss"
GAME = ROOT / "scratch" / "mirewarden-game"
BASE = GAME / "model-test-output"
CASE = BASE / "case-d46593e4d9774c82bbd33e606ec77dc6" / "case-result.json"
SETUP = BASE / "case-1f438b6751eb4ced96e9cbc035e21028" / "result.json"
SESSION = "9927e419cda1403481d25b8db1a4be04"
...
REVIEW_SPECS = (
    ...
    ("fixture-death", "kill-fixture", 40, "Defeated body remains coherent during native cleanup progression."),
    ("fixture-death", "kill-fixture", 60, "Late collapse/cleanup progression before renderer destruction ends the capture."),
    ("ready-transition", "kill-fixture", 90, "Post-removal progression frame; the renderer is no longer active and strict Ready follows."),
)
...
assert profile == {
    ...
    "displayName": "Rivenquill Cockatrice",
    "minimumBaseHealth": 64,
    "combatProfile": "b5d1919dda0eb14fc093007013c70f4441308081495026b8f5e5b4bb8eeada2c",
    "renderers": [{
...
assert registered[0]["actualBaseHealth"] == 600
...
for action_name, label, scope in action_specs:
    ...
    if action_name in {"pass", "attack"}:
        ...
        assert "Renderer destroyed during capture" in raw["error"]
        assert boundary["partial"] is True and boundary["allowRendererDestroyedPrefix"] is True
        assert boundary["retainedFrameCount"] == len(frames) == 91
        assert all(frame["isVisible"] is True and frame["active"] is True for frame in frames[:-1])
        assert frames[-1]["isVisible"] is False and frames[-1]["active"] is False
...
review = {
    ...
    "reviewStatus": "reviewed_resource_enbaseycockatriceboss_binding_motion_gameplay_observed_camera_fit_accepted_limited_art_pending",
    "session": SESSION,
    "scope": "One fresh isolated bossCockatrice / enbaseycockatriceboss / enBaseyCockatrice source pair only.",
    "binding": binding,
    ...
    "explicitKillFixture": {
        ...
        "accepted": True,
        "nativeTrigger": "Death",
        "captureTermination": "renderer_destroyed_after_91_retained_frames",
    ...
    "limits": [
        ...
        "The death capture intentionally ends when native cleanup destroys the renderer after 91 retained frames; it is recorded as an expected partial capture, not a complete 120-frame death capture.",
        ...
        "This boss source pair does not transfer to cockatriceC / enbaseycockatricesmall or any direct Cockatrice renderer row.",
    ],
}
