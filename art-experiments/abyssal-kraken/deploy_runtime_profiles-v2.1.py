#!/usr/bin/env python3
"""Review and, only with --execute, deploy a staged V2.1 emission correction."""
...
assert receipt["status"] == "STAGED_NOT_DEPLOYED_ABYSSAL_KRAKEN_V2_1_EMISSION_CORRECTION"
...
assert files(models) == receipt["assetSha256"]
...
review = {
    ...
    "preservedAssetCount": len(receipt["assetSha256"]),
    "pinnedV2Assets": receipt["pinnedV2Assets"],
...
assert_game_stopped(GAME)
backup = GAME / "deployment-backups" / ("abyssal-kraken-v2.1-" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
