...
public sealed partial class RuntimeModelTest
{
    ...
    sealed class KrakenModernInputMixer
    {
        ...
        public const string Mode="two-clip-raw-v1";
        ...
        public void Create(GameObject source,RuntimeAnimatorController native,string id)
        {
            ...
            graph=PlayableGraph.Create();graphDisposed=false;graph.SetTimeUpdateMode(DirectorUpdateMode.Manual);mixer=AnimationMixerPlayable.Create(graph,2,false);
            Replace(0,clips["krakenIdle"]);Replace(1,clips["krakenIdle"]);mixer.SetInputWeight(0,1);mixer.SetInputWeight(1,0);
            ...
            root.SetActive(true);animator.Rebind();graph.Play();inputs[0].SetTime(0);inputs[1].SetTime(0);graph.Evaluate(0);Audit();
            identity=new JObject{{"mode",Mode},{"rootInstanceId",rootId},{"animatorInstanceId",animatorId},{"avatarInstanceId",avatar.GetInstanceID()},{"avatarIsHuman",avatar.isHuman},
        ...
        void Audit()
        {
            ...
            if(root==null || !root.activeInHierarchy || animator==null || !animator.enabled || !animator.isInitialized || animator.avatar!=avatar || animator.runtimeAnimatorController!=null || animator.fireEvents || animator.applyRootMotion || animator.cullingMode!=AnimatorCullingMode.AlwaysAnimate
                || !graph.IsValid() || graph.GetTimeUpdateMode()!=DirectorUpdateMode.Manual || graph.GetPlayableCount()!=3 || graph.GetOutputCount()!=1 || !mixer.IsValid() || mixer.GetInputCount()!=2)
                throw new InvalidOperationException("Controller-free mixer ownership/configuration changed.");
            for(int i=0;i<2;i++)if(!inputs[i].IsValid() || inputs[i].GetApplyFootIK() || inputs[i].GetSpeed()!=0 || !mixer.GetInput(i).GetHandle().Equals(inputs[i].GetHandle()))throw new InvalidOperationException("Mixer input graph changed.");
        }
        JObject Evaluate(Input first,Input next)
        {
            ...
            mixer.SetInputWeight(0,first.weight);mixer.SetInputWeight(1,next==null?0:next.weight);Audit();graph.Evaluate(0);Audit();
        ...
        public JObject Step(KrakenControllerSurface modern,KrakenControllerSurface old,JObject frozenNative)
        {
            ...
            return new JObject{{"mode",Mode},{"eligibleMainComparison",!appearance},{"appearanceZeroWeightBoundary",zeroAppearance},
                ...
                {"inputWeights",new JArray(mixer.GetInputWeight(0),mixer.GetInputWeight(1))},{"playableTimes",new JArray(inputs[0].GetTime(),inputs[1].GetTime())},{"resetCount",resetCount},
        ...
        public JObject View(){return new JObject{{"mode",Mode},{"identity",identity},{"eligibleFrames",eligibleFrames},{"excludedAppearanceFrames",excludedFrames},{"resets",resetCount},{"failedMainFrames",failedMainFrames},{"failedRepeatFrames",failedRepeatFrames},{"maximumMainError",maximumMainError},{"maximumRepeatError",maximumRepeatError},{"reportedNumericalComparisonPassed",eligibleFrames>0 && failedMainFrames==0 && failedRepeatFrames==0},{"graphDisposed",graphDisposed},{"targetUnityNull",root==null},
