...
import numpy as np
OUT=Path(__file__).resolve().parent;ROOT=OUT.parent.parent;target=ROOT/'scratch/thistlewick-original-proof';target.mkdir(exist_ok=True)
...
files=[]
for name in ['thistlewick.source.json','thistlewick_basecolor.png','thistlewick.pieces.json']:
