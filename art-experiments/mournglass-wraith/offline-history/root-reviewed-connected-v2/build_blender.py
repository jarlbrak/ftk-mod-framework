"""Create editable armature scenes, independently re-export, and render original Mournglass Wraith."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/mournglass-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 for slot in range(2):
  image=bpy.data.images.load(str(OUT/f'mournglass.slot{slot}.png'),check_existing=True);image.pack()
 ...
 return obj
for rid,name in [(121008,'mournglass')]:
 ref=ROOT/'scratch/skeleton-audit/121008';create(ref/'reference.npz',ref/'skeleton.json',SCRATCH/f'{name}-template.blend',PYTHON,False)
 arm=next(o for o in bpy.data.objects if o.type=='ARMATURE')
 for obj in list(bpy.data.objects):
  if obj.type=='MESH':bpy.data.objects.remove(obj,do_unlink=True)
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Mournglass Wraith surface; live validation pending'
 ...
 bpy.ops.wm.open_mainfile(filepath=str(OUT/f'{name}.blend'));export(OUT/'mournglass-reopened.glb',native_material_slots=2)
 validation=subprocess.run([str(PYTHON),str(ROOT/'tools/ai-model-pipeline/validate_glb.py'),str(OUT/'mournglass-reopened.glb'),'--reference',str(ref/'reference.npz')],capture_output=True,text=True,check=True)
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'mournglass.blend'))
for obj in bpy.context.scene.objects:
 if obj.type=='ARMATURE':obj.hide_render=True;obj.hide_set(True)
scene=bpy.context.scene;scene.render.fps=12;scene['ftk_art_status']='Studio presentation: export original from mournglass.blend'
...
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=3.0
...
cam.location=(3,-7,3);aim(cam,(0,0,1.4));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-7,-1,3.2);aim(cam,(0,0,1.4));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(3,-7,3);aim(cam,(0,0,1.4));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'mournglass-studio.blend'))
