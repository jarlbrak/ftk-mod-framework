#!/usr/bin/env python3
"""Write the human-reviewed still-frame verdict for the fresh focused Roc trial."""
import hashlib
import json
import os
...
OUT = ROOT / 'scratch/sunspire-roc-root-visual-review-v1.json'
...
assert case['session'] == 'c244446c2d9443759ce4a67cf7eaa022'
...
assert actions['attack']['hpOutcome']['beforeHp'] == 81 and actions['attack']['hpOutcome']['afterHp'] == 71
...
choices = [
    ('pass', 0, 'Initial combat view: the dark faceted body, hooked ivory beak, paired wings, talons, and separated tail plumes read together as one Roc silhouette.'),
    ('pass', 60, 'Native pass sequence: the body remains connected while the wing pose opens, with no apparent wing or tail mesh explosion in this combat-distance sample.'),
    ('attack', 0, 'Before the focused player action resolves: the original head, face, chest mantle, wings, feet, and tail are all visible on the exact spawned owner.'),
    ('attack', 24, 'Visible 10-point hit sample: native impact effects overlay the torso, but the articulated wing and body surfaces remain continuous.'),
    ('attack', 65, 'Native Roc attack motion: legs, talons, wing roots, extended wing sections, and head stay attached during the airborne pose.'),
    ('attack', 90, 'Later attack sample: the head/beak, broad wings, and tail continue to read as a coherent bird rather than detached static decorations.'),
    ('kill-fixture', 25, 'Death-sequence entry immediately before the heavy-death clip: the still-visible original silhouette remains intact.'),
    ...
    ('kill-fixture', 119, 'Late fixture sample: native victory and loot UI obscure much of the frame, while the retained Roc remains visible behind it. This does not establish final corpse or resource lifetime.'),
]
frames = []
...
review = {
    'status': 'SCOPED_LIVE_VISUAL_REVIEW_PASS_FOR_FRESH_ORIGINAL_ROC',
    ...
    'focusedHit': {
        ...
        'enemyHpBefore': 81,
        'enemyHpAfter': 71,
        'meaning': 'Fresh nonlethal same-target damage was observed. This is explicitly focused evidence.'
    },
    'fixtureDeath': {
    ...
    'frames': frames,
    'conclusion': 'The reviewed fresh exact-owner samples show a readable original Roc whose head, beak, torso, wings, talons, and tail stay visually connected across idle/pass, focused hit, attack, and the visible death sequence. No obvious detached skinning, open panel inversion, or mesh explosion appears in these selected in-game frames.',
    'limits': [
