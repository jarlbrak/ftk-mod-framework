#!/usr/bin/env python3
"""Stage the V3 Abyssal Crown composite correction without changing the game.

The baseline already contains the separately archived V3 Sea King correction.
This stage changes only the modern Kraken head row by adding one exact static
MeshRenderer assignment for the rigid eye child. It retains all prior assets.
"""
...
BASELINE = "runtime-profiles-v3-seaking.json"
CANDIDATE = "runtime-profiles.json"
CHANGED_KEY = "ftkmf_modeltest_abyssal_crown_kraken_head"
NEW_ASSETS = {"abyssal-crown-kraken-eye-v3.glb", "abyssal-crown-kraken-eye-v3.png"}
...
def expected_head_v3_row(old: dict) -> dict:
    assert old["displayName"] == "Abyssal Crown V2"
    ...
    new["displayName"] = "Abyssal Crown V3"
    new["renderers"].append({
        "rendererPath": STATIC_PATH,
        "rendererKind": "MeshRenderer",
        "glbFile": "abyssal-crown-kraken-eye-v3.glb",
        "textureFile": "abyssal-crown-kraken-eye-v3.png",
        "disableNativeEmission": True,
    })
    return new
...
assert changes[0][1] == expected_head_v3_row(changes[0][0]), "Unexpected V3 head correction."
...
assert not asset_names(old_profiles) - asset_names(new_profiles)
...
for old, new in zip(old_profiles, new_profiles):
    ...
    index = matches[0]
    assert rows[index] == old, f"Current row drifted from V3 Sea King baseline: {old['key']}"
...
for name in (
    "runtime-profiles-v1.json", "runtime-profiles-v2.json", "runtime-profiles-v2.1.json", "runtime-profiles-v3-seaking.json", CANDIDATE,
    ...
    "abyssal-crown-kraken-eye-v3-blend-validation.json", "stage_runtime_profiles-v3-seaking.py", "deploy_runtime_profiles-v3-seaking.py",
):
...
receipt = {
    "status": "STAGED_NOT_DEPLOYED_ABYSSAL_KRAKEN_V3_HEAD_STATIC_EYE",
    ...
    "authoringFiles": {name: sha(ART / name) for name in (
        ...
        "build_static_eye.py", "build_static_eye_blender.py", "abyssal-crown-kraken-eye-v3.source.json",
        "abyssal-crown-kraken-eye-v3.pieces.json", "abyssal-crown-kraken-eye-v3.validation.json",
        "abyssal-crown-kraken-eye-v3-blend-validation.json",
    )},
