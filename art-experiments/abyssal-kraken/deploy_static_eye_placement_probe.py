#!/usr/bin/env python3
"""Review and, only with --execute, deploy the isolated static-eye placement probe."""
...
assert receipt["status"] == "STAGED_NOT_DEPLOYED_ABYSSAL_KRAKEN_STATIC_EYE_PLACEMENT_PROBE_V1"
...
assert_game_stopped(GAME)
backup = GAME / "deployment-backups" / ("abyssal-kraken-static-eye-placement-probe-v1-" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
