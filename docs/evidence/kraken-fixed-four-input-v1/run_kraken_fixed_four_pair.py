...
def main():
 ...
 def check():
  ...
  assert pin(m.ROOT/'BepInEx/plugins/FtkRuntimeModelTest.dll')['sha256']=='43529c758f91e89ab0ab10d83b00499d725f33709e903b3cf81c101d1b727ee1'
 ...
 out=Path('scratch')/('kraken-fixed-four-'+a.scenario+'-live-v1');out.mkdir();save(out/'before-ready.json',before);shutil.copy2(receipt_path,out/'deployment.json')
 for label in ['first','repeat']:
  check();ready=m.cmd('fixture-state');assert ready['strictReady']==before['strictReady']
  key=m.send('kraken-controller-fixture',scenario=a.scenario,modernInputMixer='fixed-four-raw-v1');shutil.copy2(m.ROOT/'model-test-command.json',out/(label+'-request.json'))
