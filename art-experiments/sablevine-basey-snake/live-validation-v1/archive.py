#!/usr/bin/env python3
"""Archive the reviewed Sablevine resource-prefab trial without game payloads."""
...
CASE = BASE / "case-29f0739623aa40a28c2f229b0b7381f3" / "case-result.json"
SETUP = BASE / "case-abe1a932ff03438a9da7e9ca22ad62d7" / "result.json"
REVIEW = ROOT / "scratch" / "sablevine-root-visual-review-v1.json"
...
def load_summarizer():
    ...
    spec = importlib.util.spec_from_file_location("sablevine_capture_summary", module_path)
...
assert case["enemy"] == "ftkmf_modeltest_sablevine_basey_snake"
...
assert runtime_profile["profiles"][0]["baseEnemy"] == "snakeJungleA"
assert runtime_profile["profiles"][0]["resourcePrefab"] == "enbaseysnake"
assert setup["status"] == "binding_metadata_observed"
assert len(setup["matches"]) == 1
assert setup["matches"][0]["rendererPath"] == "enSnake_Basey"
assert setup["matches"][0]["glbFile"] == "sablevine.glb"
...
assert actions["pass"]["motionEvidence"]["attack"]["nativeTrigger"]["trigger"] == "Attack"
assert actions["attack"]["motionEvidence"]["hit"]["nativeTrigger"]["trigger"] == "Damaged"
assert actions["attack"]["motionEvidence"]["hit"]["nativeAction"]["primary"]["newHealth"] == 50
