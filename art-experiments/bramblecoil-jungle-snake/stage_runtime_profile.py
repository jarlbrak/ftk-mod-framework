#!/usr/bin/env python3
"""Append frozen Bramblecoil to one fresh isolated test-catalog stage."""
import argparse
import copy
import hashlib
import json
...
REPO = args.repo.resolve() if args.repo else next(path for path in Path(__file__).resolve().parents if (path / "FTKModFramework").is_dir() and (path / "tools").is_dir())
ART = REPO / "art-experiments/bramblecoil-jungle-snake"
...
assert manifest["status"] in {"FROZEN_OFFLINE_ORIGINAL_CANDIDATE_LIVE_VALIDATION_PENDING", "FROZEN_ORIGINAL_CANDIDATE_WITH_SCOPED_LIVE_EVIDENCE"}
...
probe = next(row for row in old["profiles"] if row["key"] == "ftkmf_modeltest_probe_snakejunglec")
assert probe["baseEnemy"] == profile["baseEnemy"] == "snakeJungleC"
assert probe["combatProfile"] == profile["combatProfile"]
assert probe["renderers"][0]["rendererPath"] == profile["renderers"][0]["rendererPath"] == "enJungleSnakeC"
