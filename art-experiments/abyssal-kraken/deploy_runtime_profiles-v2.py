#!/usr/bin/env python3
"""Review and, only with --execute, deploy a staged V2 Kraken correction."""
...
assert receipt["status"] == "STAGED_NOT_DEPLOYED_ABYSSAL_KRAKEN_V2_CORRECTION"
...
review = {
    ...
    "v1AssetsRetained": receipt["v1AssetsRetained"],
...
assert_game_stopped(GAME)
backup = GAME / "deployment-backups" / ("abyssal-kraken-v2-" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
