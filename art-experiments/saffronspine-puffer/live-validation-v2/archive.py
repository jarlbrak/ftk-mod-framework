#!/usr/bin/env python3
"""Archive the observed Saffronspine A live trial without game payloads."""
...
ASSET = ROOT / "art-experiments" / "saffronspine-puffer"
OUT = ASSET / "live-validation-v2"
BASE = ROOT / "scratch" / "mirewarden-game" / "model-test-output"
CASE = ROOT / "scratch" / "saffronspine-a-standalone-case.json"
REVIEW = ROOT / "scratch" / "saffronspine-a-root-visual-review.json"
...
assert case["enemy"] == "ftkmf_modeltest_saffronspine"
assert len(case["actions"]) == 3
assert len(case["sessions"]) == 1
assert review["binding"]["nativeChassis"] == "pufferA"
assert review["binding"]["ownerInstanceId"] == 369188
assert review["binding"]["renderers"] == [{
    "rendererPath": "enBlowFishA",
    "rendererId": 121509,
    "capturedInstanceId": -245400,
    "glbFile": "saffronspine.glb",
    "boneSignature": "8198a1e60d15c1dd19d32b83be60423c80eb129cbc9b48d273c450fac78f716c",
}]
...
validation = {
    "status":"fresh_catalog_411_single_process_pufferA_binding_complete_attack_kill_loot_ready_observed",
    ...
    "progression":"The native pass remains a pufferA pass/attack boundary and is not classified as death. A separate ordinary attack resolves HP 58→48 without a cheat. The same fresh process then supplies the explicit BlowFish_DeathDirect kill capture, two guarded native Collect clicks and strict Ready at level 0 room 2.",
...
(OUT / "README.md").write_text("""# Saffronspine Puffer fresh live trial V2 (pufferA)

This supplement records one fresh catalog-411 process for the exact pufferA chassis, renderer enBlowFishA, controller 5999, native CEL scale 1.0, and the authored saffronspine.glb with its observed 30-bone signature. The custom mesh bound under one live enemy owner (369188) and the authored basecolor was read back on the native Standard material with native emission disabled.

The process preserves the native puffer pass/attack behavior. The ordinary attack capture resolves the target from HP 58 to 48 without cheat or focus and is an appearance/motion record, not a lethal or death-animation verdict. The explicit KillSingle fixture then records BlowFish_DeathDirect across a complete 120-frame capture. Native Victory loot accepts two guarded Collect votes, with the first changing gold 11→25, and strict Ready is observed at level 0 / room 2 after one guarded Ready vote.
