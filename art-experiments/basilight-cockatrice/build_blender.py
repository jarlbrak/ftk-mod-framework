"""Create editable armature scenes, independently re-export, and render original Basilight Cockatrice."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/basilight-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'basilight_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Basilight Cockatrice original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121328,'basilight')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Basilight Cockatrice surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'basilight.blend'))
for obj in bpy.context.scene.objects:
 if obj.type=='ARMATURE':obj.hide_render=True;obj.hide_set(True)
scene=bpy.context.scene;scene.render.fps=12;scene['ftk_art_status']='Studio presentation: export original from basilight.blend'
...
def light(name,loc,power,color,size):
 data=bpy.data.lights.new(name,'AREA');data.energy=power;data.color=color;data.shape='DISK';data.size=size;obj=bpy.data.objects.new(name,data);scene.collection.objects.link(obj);obj.location=loc;aim(obj,(0,1.2,2.5))
light('Warm key',(6,-8,12),1500,(.82,.90,1),4);light('Cool fill',(-8,-3,7),1100,(.59,.78,1),3);light('Copper rim',(0,7,10),1800,(.70,.62,1),2)
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=12.5
...
cam.location=(9,-14,11);aim(cam,(0,1.5,2.3));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-16,-2,8);aim(cam,(0,1.5,2.3));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(9,-14,11);aim(cam,(0,1.5,2.3));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'basilight-studio.blend'))
