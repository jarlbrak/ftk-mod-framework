"""Create editable armature scenes, independently re-export, and render original Moonreed Sylph."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/moonreed-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'moonreed_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Moonreed Sylph original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121395,'moonreed')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Moonreed Sylph surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'moonreed.blend'))
...
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=2.05
...
cam.location=(2.1,-5.2,1.9);aim(cam,(0,.05,.79));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-5.0,-1.5,1.5);aim(cam,(0,.05,.79));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(2.1,-5.2,1.9);aim(cam,(0,.05,.79));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'moonreed-studio.blend'))
