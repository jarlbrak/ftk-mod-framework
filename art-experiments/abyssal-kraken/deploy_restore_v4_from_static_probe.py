#!/usr/bin/env python3
"""Review and, only with --execute, restore the V4 compact eye in the isolated game."""
...
EXPECTED_STATUS = "STAGED_NOT_DEPLOYED_ABYSSAL_KRAKEN_V4_RESTORE_FROM_PROBE"
...
assert receipt["newAssets"] == {}
for name, digest in receipt["existingCandidateAssets"].items():
    assert sha(models / name) == digest and sha(staged_models / name) == digest, name
...
review = {
    ...
    "changes": expected,
    "assetChanges": {},
    "existingCandidateAssets": receipt["existingCandidateAssets"],
...
assert_game_stopped(GAME)
backup = GAME / "deployment-backups" / ("abyssal-kraken-v4-restore-from-probe-" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
...
try:
    ...
    deployment.write_text(json.dumps(record, indent=2) + "\n")
    raise
print(json.dumps({"status": record["status"], "deployment": str(deployment), "new": expected, "assetChanges": {}}, indent=2))
