...
def write_texture(filename: str, theme: str, shift: int = 0) -> None:
    ...
    if theme == "head":
        ...
        # Sea King V2 used a shifted generic palette. Under the game's
        # inherited Standard lighting it became a yellow bamboo stripe even
        # after emission was removed. This authored field stays in the deep
        # blue-green range, reserving gold for small growth details only.
        dark, mid, light = rgb(1), rgb(2), rgb(14)
        field = .18 + .24 * base
        paint = dark[None, None, :] * (1.0 - field[..., None]) + mid[None, None, :] * field[..., None]
        for phase in (.12, .39, .67, .88):
            curve = phase + .038 * np.sin(v * math.pi * (3.2 + phase * 2.0) + phase * 9.0)
            mask = np.exp(-((u - curve) ** 2) / .00125)
            paint = paint * (1.0 - .38 * mask[..., None]) + light[None, None, :] * (.38 * mask[..., None])
        for center in (.18, .43, .69, .88):
            mask = np.exp(-((v - center) ** 2) / .000032)
            growth = rgb(7) * .42 + mid * .58
            paint = paint * (1.0 - .24 * mask[..., None]) + growth[None, None, :] * (.24 * mask[..., None])
        flecks = ((np.sin((u * 1.4 + v) * math.pi * 31.0) + np.sin((u - v * .63) * math.pi * 37.0)) > 1.82)
        paint[flecks] = paint[flecks] * .64 + rgb(5) * .36
    else:
...
def build_tentacle(renderer_id: int, name: str, texture: str, native_chassis: str, renderer_path: str, style_shift: int,
                   theme: str = "tentacle", contour: float = 1.0) -> dict:
    write_texture(texture, theme, shift=style_shift)
    ...
    width = [contour * .88 * ((1.0 - value) ** .68) + .075 for value in progress]
    depth = [radius * .72 for radius in width]
    skins = [chain[0]] + [blend(chain[index - 1], chain[index], .62) for index in range(1, len(chain))]
    palette = ([1, 2, 14, 2, 1, 14, 2] if theme == "royal" else
               [1 + style_shift, 2 + style_shift, 3 + style_shift, 4 + style_shift, 15 + style_shift, 4 + style_shift, 3 + style_shift])
    ...
    for strand, phase in enumerate((0.0, 2.0 * math.pi / 3.0, 4.0 * math.pi / 3.0), 1):
        ...
        spiral_palette = ([2, 14, 5] if theme == "royal" else [4 + style_shift + strand, 5 + style_shift + strand, 7 + style_shift])
        surface.tube(f"Bioluminescent spiral strand {strand}", braid_points,
                     ...
                     skins, spiral_palette, axis=(1, 0, 0), sides=7, uv_wrap=True, uv_offset=(strand * .17, .06), uv_scale=(.78, .92))
...
def main() -> None:
    reports = [
        ...
        build_tentacle(121315, "royal-sargassum-seaking-tentacle-v3", "royal-sargassum-seaking-tentacle-v3.png", "seaKingTentacleA/B", "KrakenGodTentacle", 0, theme="royal", contour=.74),
    ]
    ...
    print(json.dumps({"status": "PASS_ORIGINAL_KRAKEN_GEOMETRY_EXPORT", "assets": [report["asset"] for report in reports], "revision": "V3 Sea King palette and contour correction after V2.1 live visual rejection"}, indent=2))
