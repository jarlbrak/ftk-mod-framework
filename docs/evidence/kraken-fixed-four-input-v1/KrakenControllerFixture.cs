...
public sealed partial class RuntimeModelTest
{
    ...
    {
        const float stepSeconds=1f/60f;const int steps=240,triggerAfterStep=15;
        ...
        try
        {
            try
            {
                ...
                {
                    if(command["modernInputMixer"].Type!=JTokenType.String || ((string)command["modernInputMixer"]!=KrakenModernInputMixer.Mode && (string)command["modernInputMixer"]!=KrakenModernInputMixer.FixedMode) || command["endpointPolicy"]!=null || command["observeExistingDrivers"]!=null || krakenSkinArm!=null)
                ...
                {
                    if(command["endpointPolicy"].Type!=JTokenType.String || (string)command["endpointPolicy"]!="main-appearance-local-v1")
                        throw new ArgumentException("Only endpointPolicy main-appearance-local-v1 is supported.");
                ...
                identity=new JObject{{"resourcesSha256",KrakenControllerAssets},{"gameAssembly",assembly},{"controllerName",native.name},
                    ...
                    {"targetStateHash",Animator.StringToHash("Base Layer."+KrakenScenarioState(trigger))},{"sourceProof","21 default-false triggers; five one-trigger paths exclude all three attack SMB ranges; no custom creation or machine callbacks"}};
                ...
                if(modernMixer!=null)modernMixer.Create(modernSource,native,id,(string)command["modernInputMixer"]);
                if(old.observeExistingDrivers)identity["oldExistingDriverRest"]=old.driverRest;
                if(endpoint!=null)endpoint.Create(modernSource,oldSource,native,scenario,id);
            ...
            if(error==null)for(int step=0;step<=steps;step++)
            {
                try
                {
                    ...
                    if(endpoint!=null)frame["endpointPolicy"]=endpoint.Step(modern,old);
                    if(skin!=null)
                    {
        ...
        try
        {
            ...
            JObject result=new JObject{{"ok",error==null && engineErrors.Count==0 && sameReady && cleanup && frames.Count==steps+1 && (skin==null || skin.Complete())},{"error",error},
                {"provenance",endpoint==null?"owned_native_controller_graph_observation_no_adapter":"owned_controller_endpoint_policy_diagnostic"},{"endpointPolicy",endpoint==null?null:command["endpointPolicy"]},{"endpointRests",endpoint==null?null:endpoint.rests},{"pureEndpointChecks",endpoint==null?0:endpoint.pureChecks},{"scenario",scenario},{"identity",identity},
