...
import numpy as np
OUT=Path(__file__).resolve().parent;ROOT=OUT.parent.parent;target=ROOT/'scratch/emberglass-original-proof';target.mkdir(exist_ok=True)
...
files=[]
for name in ['emberglass.source.json','emberglass_basecolor.png','emberglass.pieces.json']:
