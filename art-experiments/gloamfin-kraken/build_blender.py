"""Create editable armature scenes, independently re-export, and render original Gloamfin Kraken."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/gloamfin-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'gloamfin_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Gloamfin Kraken original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121260,'gloamfin')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Gloamfin Kraken surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'gloamfin.blend'))
...
bpy.ops.mesh.primitive_plane_add(size=200,location=(0,0,-5.75));floor=bpy.context.object;floor.name='Studio floor - not export';floor['ftk_export']=False
...
def light(name,loc,power,color,size):
 data=bpy.data.lights.new(name,'AREA');data.energy=power;data.color=color;data.shape='DISK';data.size=size;obj=bpy.data.objects.new(name,data);scene.collection.objects.link(obj);obj.location=loc;aim(obj,(0,1,-.5))
light('Warm key',(5,-7,8),1500,(1,.79,.57),6);light('Cool fill',(-8,-4,3),1200,(.59,.78,1),6);light('Copper rim',(0,7,6),1800,(1,.43,.18),5)
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=12.2
...
cam.location=(7,-13,4);aim(cam,(0,1.2,-.65));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-12,-7,2);aim(cam,(0,1.2,-.65));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(7,-13,4);aim(cam,(0,1.2,-.65));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'gloamfin-studio.blend'))
