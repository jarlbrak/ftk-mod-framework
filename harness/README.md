...
- **Run / flow**: `start_run {adventure?}` (autonomous title -> in-world),
  ...
  flag (`LeaveOrEndTurn`); otherwise it drives the hourglass path directly
  (`FTKHub.EndTurn`), so a turn can be ended from a plain hex. `FTKHub.EndTurn`
  only acts in certain movement-FSM states, so when the end cannot be confirmed
  the result reports the observed state as `fsmState`. Ending a turn is what
  advances rounds/time (and with it the per-turn encounter spawn roll).
