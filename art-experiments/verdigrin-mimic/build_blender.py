"""Create editable armature scenes, independently re-export, and render original Verdigrin Coffer."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/verdigrin-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'verdigrin_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Verdigrin Coffer original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121192,'verdigrin')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Verdigrin Coffer surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'verdigrin.blend'))
...
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=2.70
...
cam.location=(2.6,-5.2,2.9);aim(cam,(0,0,.99));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-4.8,-2.1,2.4);aim(cam,(0,0,.99));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(2.6,-5.2,2.9);aim(cam,(0,0,.99));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'verdigrin-studio.blend'))
