"""Create editable armature scenes, independently re-export, and render original Briarback Guardian."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/briarback-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'briarback_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Briarback Guardian original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(120991,'briarback')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Briarback Guardian surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'briarback.blend'))
...
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=5.1
...
cam.location=(-4.8,-2.4,3.1);aim(cam,(0,0,1.65));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(3.1,-7,3.5);aim(cam,(0,0,1.65));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'briarback-studio.blend'))
