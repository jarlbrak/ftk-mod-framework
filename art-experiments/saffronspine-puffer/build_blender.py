...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/saffronspine-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'saffronspine_basecolor.png'),check_existing=True);image.pack()
 ...
 return obj
for rid,name in [(121509,'saffronspine')]:
 ref=ROOT/'scratch/puffer-native-topology-analysis/reference-121509';create(ref/'reference.npz',ref/'skeleton.json',SCRATCH/f'{name}-template.blend',PYTHON,False)
 ...
 bpy.ops.wm.open_mainfile(filepath=str(OUT/f'{name}.blend'));export(OUT/'saffronspine-reopened.glb')
 validation=subprocess.run([str(PYTHON),str(ROOT/'tools/ai-model-pipeline/validate_glb.py'),str(OUT/'saffronspine-reopened.glb'),'--reference',str(ref/'reference.npz')],capture_output=True,text=True,check=True)
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'saffronspine.blend'))
for obj in bpy.context.scene.objects:
 if obj.type=='ARMATURE':obj.hide_render=True;obj.hide_set(True)
scene=bpy.context.scene;scene.render.fps=12;scene['ftk_art_status']='Studio presentation: export original from saffronspine.blend'
...
cam.location=(3,-7,3);aim(cam,(0,.3,1.55));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'saffronspine-studio.blend'))
