"""Create editable armature scenes, independently re-export, and render original Vesper Eye."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/vesper-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'vesper_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Vesper Eye original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121031,'vesper_body'),(121210,'vesper_eye')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Vesper Eye surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'vesper_body.blend'))
with bpy.data.libraries.load(str(OUT/'vesper_eye.blend'),link=False) as (source,target):target.objects=source.objects
...
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=3.65
...
cam.location=(2.6,-6,3.3);aim(cam,(0,0,1.75));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'vesper-studio.blend'))
