...
for name,rid in [('emberglass',121062)]:
 ...
 v=json.loads((OUT/f'{name}.validation.json').read_text());b=json.loads((ROOT/f'scratch/emberglass-roundtrip/{name}.validation.json').read_text())
...
manifest['art_status']='Original amber insect surface model; manual studio and live acceptance pending after regeneration'
...
if review_path.exists():
 ...
 if all(sha(OUT/name)==expected for name,expected in review['images'].items()):
  ...
  manifest['art_status']='Original amber insect; studio approved for live test, original live acceptance pending'
