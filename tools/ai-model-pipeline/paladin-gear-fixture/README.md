...
three legendary items plus twelve accessories. Only the package's
...
- Existing class `paladin`: replace only `fields.startitems` with all 51 equipment
...
- Add `paladin_gear_fixture_hunter`: `kind: class`, `template: hunter`, display name
  ...
  51 `startitems`. The native Hunter appearance and starting weapon remain
  inherited. No Paladin model plan or Guard icon is attached to this class.

The 48 progression keys are `paladin_{family}_{set}` for families `hammer_1h`, `hammer_2h`,
`shield`, `armor`, `boots`, `helmet`, `trinket`, `necklace` and sets `novice`, `oathkeeper`, `highward`,
`mercy`, `censure`, `verdict`. The three Artifact keys are
`paladin_hammer_1h_last_vigil`, `paladin_hammer_2h_kingsfall` and
`paladin_shield_last_bastion`. The fixture has 55 rows, versus the production 54.
...
fixture checks every reference against its 51 authored item/weapon rows. It uses
