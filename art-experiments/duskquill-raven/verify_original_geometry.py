...
import numpy as np
OUT=Path(__file__).resolve().parent;ROOT=OUT.parent.parent;target=ROOT/'scratch/duskquill-original-proof';target.mkdir(exist_ok=True)
...
files=[]
for name in ['duskquill.source.json','duskquill_basecolor.png','duskquill.pieces.json']:
