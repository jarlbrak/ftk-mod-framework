...
for(const size of [{width:1440,height:1000},{width:390,height:844}]){
 ...
 for(const path of paths){
  ...
  const info=await page.evaluate(()=>({images:[...document.images].map(i=>({src:i.src,alt:i.alt,loaded:i.complete&&i.naturalWidth>0})),links:[...document.querySelectorAll('a[href]')].map(a=>a.href),media:[...document.querySelectorAll('source')].map(s=>s.src),videos:[...document.querySelectorAll('video')].map(v=>({controls:v.controls,autoplay:v.autoplay,poster:v.poster,loop:v.loop}))}));
...
await page.emulateMedia({reducedMotion:'reduce'});await page.goto(root+'gallery/');
