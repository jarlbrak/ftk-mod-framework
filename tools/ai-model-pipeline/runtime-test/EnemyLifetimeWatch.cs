...
public sealed partial class RuntimeModelTest
{
    ...
    JObject EnemyLifetimeRegistration(string enemy)
    {
        ...
        {if(assignment==null)throw new InvalidOperationException("Null registered assignment.");Type t=assignment.GetType();object kind=t.GetProperty("RendererKind",Members).GetValue(assignment,null);JObject descriptor=new JObject{{"rendererPath",(string)t.GetProperty("RendererPath",Members).GetValue(assignment,null)},{"glbFile",(string)t.GetProperty("GlbFileName",Members).GetValue(assignment,null)},
            {"textureFile",new JValue((object)(string)t.GetProperty("TextureFileName",Members).GetValue(assignment,null))},{"disableNativeEmission",(bool)t.GetProperty("DisableNativeEmission",Members).GetValue(assignment,null)},{"rendererKind",Convert.ToString(kind)}};
    ...
    static string EnemyLifetimeRendererKind(JObject assignment)
    {
        string kind=Str(assignment,"rendererKind")??"SkinnedMeshRenderer";
        if(kind!="SkinnedMeshRenderer"&&kind!="MeshRenderer")throw new InvalidOperationException("Unsupported configured renderer kind.");
        return kind;
    }
    static JArray EnemyLifetimeMaterials(Material[] materials)
    {
        if(materials==null||materials.Length>16)throw new InvalidOperationException("Material slot bound16 exceeded.");
        JArray mats=new JArray();foreach(Material material in materials)
    ...
    JObject EnemyLifetimeCel(CharacterEventListener cel)
    {
        ...
        {
            string kind=EnemyLifetimeRendererKind(assignment);Renderer found;Mesh mesh;MeshFilter filter=null;JArray boneIds=new JArray();string signature=null;
            if(kind=="MeshRenderer")
            {
                MeshRenderer rigid=null;foreach(MeshRenderer r in cel.GetComponentsInChildren<MeshRenderer>(true))
                    if(Relative(r.transform,cel.transform)==Str(assignment,"rendererPath")){if(rigid!=null)throw new InvalidOperationException("Ambiguous static model renderer.");rigid=r;}
                MeshFilter[] filters=rigid==null?new MeshFilter[0]:rigid.GetComponents<MeshFilter>();
                if(rigid==null||filters.Length!=1||filters[0].sharedMesh==null)throw new InvalidOperationException("Configured static MeshRenderer/MeshFilter unavailable; cannot attribute body.");
                found=rigid;filter=filters[0];mesh=filter.sharedMesh;
            }
            else
            {
                SkinnedMeshRenderer skinned=null;foreach(SkinnedMeshRenderer r in cel.GetComponentsInChildren<SkinnedMeshRenderer>(true))
                    if(Relative(r.transform,cel.transform)==Str(assignment,"rendererPath")){if(skinned!=null)throw new InvalidOperationException("Ambiguous model renderer.");skinned=r;}
                if(skinned==null||skinned.sharedMesh==null)throw new InvalidOperationException("Configured native renderer unavailable; cannot attribute body.");
                ...
                {SkinnedMeshRenderer heuristic=null;foreach(Transform node in cel.GetComponentsInChildren<Transform>(true))if(node.name=="enTroll01"){heuristic=node.GetComponent<SkinnedMeshRenderer>();break;}
                    if(heuristic==null)heuristic=cel.GetComponentInChildren<SkinnedMeshRenderer>(true);if(heuristic!=skinned)throw new InvalidOperationException("Legacy native target heuristic differs from explicit observation metadata.");}
                found=skinned;mesh=skinned.sharedMesh;signature=BoneSignature(skinned);
                foreach(Transform bone in skinned.bones){if(++bones>256||bone==null||(bone!=cel.transform&&!bone.IsChildOf(cel.transform)))throw new InvalidOperationException("Bone bound/remapping failure.");boneIds.Add(bone.GetInstanceID());}
            }
            ...
            JArray mats=EnemyLifetimeMaterials(found.sharedMaterials);
            renderers.Add(new JObject{{"rendererPath",Str(assignment,"rendererPath")},{"rendererKind",kind},{"rendererId",found.GetInstanceID()},{"meshFilterId",filter==null?0:filter.GetInstanceID()},{"meshId",mesh.GetInstanceID()},{"meshName",mesh.name},{"expectedCustomMesh",mesh.name==(kind=="MeshRenderer"?"ftkmf_static_glb_":"ftkmf_glb_")+Str(assignment,"glbFile")},{"targetMember",owned},{"boneIds",boneIds},{"boneSignature",new JValue((object)signature)},{"materials",mats}});
        }
        ...
        {if(other.Count>=64)throw new InvalidOperationException("Other live renderer bound64 exceeded.");JArray materials=new JArray();Material[] shared=renderer.sharedMaterials;if(shared.Length>16)throw new InvalidOperationException("Other material slot bound16 exceeded.");foreach(Material material in shared){bool inLease=false;if(material!=null)foreach(JToken rid in resourceIds)if((int)rid==material.GetInstanceID())inLease=true;materials.Add(new JObject{{"materialId",material==null?0:material.GetInstanceID()},{"inLease",inLease}});}MeshFilter[] filters=renderer is MeshRenderer?renderer.GetComponents<MeshFilter>():new MeshFilter[0];Mesh mesh=filters.Length==1?filters[0].sharedMesh:null;bool meshInLease=false;if(mesh!=null)foreach(JToken rid in resourceIds)if((int)rid==mesh.GetInstanceID())meshInLease=true;other.Add(new JObject{{"rendererId",renderer.GetInstanceID()},{"rendererKind",renderer is MeshRenderer?"MeshRenderer":"SkinnedMeshRenderer"},{"path",Relative(renderer.transform,cel.transform)},{"active",renderer.gameObject.activeInHierarchy},{"enabled",renderer.enabled},{"meshFilterCount",filters.Length},{"meshId",mesh==null?0:mesh.GetInstanceID()},{"meshName",mesh==null?null:mesh.name},{"meshInLease",meshInLease},{"materials",materials},{"nativeScrollerCount",renderer.GetComponents<ScrollingUVs>().Length}});}
    ...
    JObject ArmEnemyLifetime(JObject command)
    {
        ...
        foreach(Renderer r in w.prefabCel.GetComponentsInChildren<Renderer>(true))
        {
            if(!(r is SkinnedMeshRenderer) && !(r is MeshRenderer))continue;
            List<UnityEngine.Object> references=new List<UnityEngine.Object>();SkinnedMeshRenderer skinned=r as SkinnedMeshRenderer;
            if(skinned!=null&&skinned.sharedMesh!=null)references.Add(skinned.sharedMesh);
            MeshRenderer rigid=r as MeshRenderer;if(rigid!=null)foreach(MeshFilter filter in rigid.GetComponents<MeshFilter>())if(filter.sharedMesh!=null)references.Add(filter.sharedMesh);
