"""Create editable armature scenes, independently re-export, and render original Rimecrown Sentinel."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/rimecrown-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'rimecrown_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Rimecrown Sentinel original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121404,'rimecrown_hat'),(121414,'rimecrown_head'),(121556,'rimecrown_base'),(121638,'rimecrown_scarf'),(121696,'rimecrown_body')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Rimecrown Sentinel surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'rimecrown_body.blend'))
for part in ['hat','head','base','scarf']:
 with bpy.data.libraries.load(str(OUT/f'rimecrown_{part}.blend'),link=False) as (source,target):target.objects=source.objects
...
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=4.65
...
cam.location=(3.1,-7,3.5);aim(cam,(0,0,1.65));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-5.8,-3,3.5);aim(cam,(0,0,1.65));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(3.1,-7,3.5);aim(cam,(0,0,1.65));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'rimecrown-studio.blend'))
