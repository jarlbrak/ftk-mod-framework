"""Create editable armature scenes, independently re-export, and render original Rustpetal Snapper."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/rustpetal-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'rustpetal_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Rustpetal Snapper original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121537,'rustpetal')]:
 ref=ROOT/'scratch/plant-d-native-topology-analysis/reference-121537';create(ref/'reference.npz',ref/'skeleton.json',SCRATCH/f'{name}-template.blend',PYTHON,False)
 arm=next(o for o in bpy.data.objects if o.type=='ARMATURE')
 for obj in list(bpy.data.objects):
  if obj.type=='MESH':bpy.data.objects.remove(obj,do_unlink=True)
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Rustpetal Snapper surface; live validation pending'
 ...
 bpy.ops.wm.open_mainfile(filepath=str(OUT/f'{name}.blend'));export(OUT/'rustpetal-reopened.glb')
 validation=subprocess.run([str(PYTHON),str(ROOT/'tools/ai-model-pipeline/validate_glb.py'),str(OUT/'rustpetal-reopened.glb'),'--reference',str(ref/'reference.npz')],capture_output=True,text=True,check=True)
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'rustpetal.blend'))
for obj in bpy.context.scene.objects:
 if obj.type=='ARMATURE':obj.hide_render=True;obj.hide_set(True)
scene=bpy.context.scene;scene.render.fps=12;scene['ftk_art_status']='Studio presentation: export original from rustpetal.blend'
...
cam.location=(3,-7,3);aim(cam,(0,0,.78));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'rustpetal-studio.blend'))
