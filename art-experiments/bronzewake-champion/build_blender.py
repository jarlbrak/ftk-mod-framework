"""Create editable armature scenes, independently re-export, and render original Bronzewake Champion."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/bronzewake-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'bronzewake_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Bronzewake Champion original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121272,'bronzewake_body'),(121500,'bronzewake_hair'),(121522,'bronzewake_armor'),(121661,'bronzewake_boots')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Bronzewake Champion surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'bronzewake_body.blend'))
for part in ['hair','armor','boots']:
 with bpy.data.libraries.load(str(OUT/f'bronzewake_{part}.blend'),link=False) as (source,target):target.objects=source.objects
...
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=2.95
...
cam.location=(2.4,-6,2.6);aim(cam,(0,0,1.05));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-4.8,-2.4,2.2);aim(cam,(0,0,1.05));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(2.4,-6,2.6);aim(cam,(0,0,1.05));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'bronzewake-studio.blend'))
