...
for name,rid in [('cinderbloom_body',120953),('cinderbloom_leaves',121072)]:
 ...
 v=json.loads((OUT/f'{name}.validation.json').read_text());b=json.loads((ROOT/f'scratch/cinderbloom-roundtrip-v2/{name}.validation.json').read_text())
 ...
 manifest['parts'].append(dict(name=name,vertices=v['vertices'],triangles=v['triangles'],palette_joints=v['bones'],max_bind_rest_error=v['max_bind_rest_error'],blender_reopen_max_bind_rest_error=b['max_bind_rest_error'],normal_agreement=v['source_normal_agreement']['positive_fraction'],bind_bounds_min=v['bounds_min'],bind_bounds_max=v['bounds_max'],blender_roundtrip_glb_sha256=sha(ROOT/f'scratch/cinderbloom-roundtrip-v2/{name}.glb')))
movie=json.loads(subprocess.check_output(['ffprobe','-v','error','-show_entries','stream=width,height,nb_frames,r_frame_rate,duration','-of','json',str(OUT/'turntable.mp4')]))['streams'][0]
assert (movie['nb_frames'],movie['r_frame_rate'])==('48','12/1')
manifest['turntable']=dict(frames=48,fps=12,seconds=4,resolution=[movie['width'],movie['height']],scope='Studio camera orbit of bind-pose art, not animated gameplay')
manifest['checks']=dict(independent_export='pass for both parts',saved_blender_reopen_export='pass for both parts',native_bind_bounds='both original surfaces contained in corresponding native rest surface bounds; not animated-envelope proof',studio_review='pending manual review after regeneration',live_binding='pending',live_animation='pending',live_material_and_culling='pending')
manifest['art_status']='Original multipart surface model; manual studio and live acceptance pending after regeneration'
