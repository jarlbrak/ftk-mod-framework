#!/usr/bin/env python3
"""Append frozen Duneshade to one fresh isolated test-catalog stage."""
import argparse
import copy
import hashlib
import json
...
ART = REPO / "art-experiments/duneshade-desert-asp"
...
probe = next(row for row in old["profiles"] if row["key"] == "ftkmf_modeltest_probe_snakedeserta")
assert probe["baseEnemy"] == profile["baseEnemy"] == "snakeDesertA"
assert probe["combatProfile"] == profile["combatProfile"]
assert probe["renderers"][0]["rendererPath"] == profile["renderers"][0]["rendererPath"] == "enDesertSnakeA"
