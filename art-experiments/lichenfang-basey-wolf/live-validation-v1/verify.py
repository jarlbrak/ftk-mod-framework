#!/usr/bin/env python3
"""Independently verify Lichenfang Prowler's immutable V1 live archive."""
...
OUT = ROOT / "art-experiments" / "lichenfang-basey-wolf" / "live-validation-v1"
...
assert validation["status"] == "reviewed_resource_enbaseywolf_binding_appearance_idle_attack_hit_death_and_gameplay_observed"
assert validation["assetHashes"] == assets
assert validation["nativeChassis"] == validation["baseEnemy"] == "wolfA"
assert validation["resourcePrefab"] == "enbaseywolf"
assert validation["rendererPaths"] == ["Wolfie"]
assert validation["profile"]["key"] == validation["enemy"] == "ftkmf_modeltest_lichenfang_basey_wolf"
...
assert by_action["pass"]["causalMotion"]["attack"]["nativeTrigger"]["trigger"] == "AttackProf"
assert by_action["attack"]["causalMotion"]["hit"]["nativeTrigger"]["trigger"] == "Damaged"
assert by_action["attack"]["causalMotion"]["hit"]["nativeAction"]["primary"]["newHealth"] == 48
...
assert binding["resourcePrefab"] == "enbaseywolf"
assert binding["rendererPath"] == "Wolfie" and binding["mesh"] == "lichenfang.glb"
assert binding["catalogSha256"] == validation["catalogSha256"]
ordinary = validation["ordinaryHit"]
assert ordinary == {
    "action": "attack", "cheat": "None", "focus": False,
    "beforeHp": 58, "afterHp": 48, "clip": "damaged_wolf",
