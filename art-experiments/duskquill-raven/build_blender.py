"""Create editable armature scenes, independently re-export, and render original Duskquill Raven."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/duskquill-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'duskquill_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Duskquill Raven original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(120960,'duskquill')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Duskquill Raven surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'duskquill.blend'))
...
light('Warm key',(2.8,-3.6,5),420,(.82,.90,1),4);light('Cool fill',(-3,-1,2),240,(.59,.78,1),3);light('Copper rim',(0,3,4),520,(.70,.62,1),2)
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=4.5
...
cam.location=(2.1,-5.2,4.0);aim(cam,(0,0,1.2));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-5.0,-1.5,2.7);aim(cam,(0,0,1.2));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(2.1,-5.2,4.0);aim(cam,(0,0,1.2));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'duskquill-studio.blend'))
