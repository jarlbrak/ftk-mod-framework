"""Create editable armature scenes, independently re-export, and render original Resinmaw Bogling."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/resinmaw-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'resinmaw_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Resinmaw Bogling original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121344,'resinmaw')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Resinmaw Bogling surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'resinmaw.blend'))
...
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=5.2
...
cam.location=(3.6,-6.2,3.6);aim(cam,(0,.6,1.55));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-5.8,-1.7,3.1);aim(cam,(0,.6,1.55));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(3.6,-6.2,3.6);aim(cam,(0,.6,1.55));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'resinmaw-studio.blend'))
