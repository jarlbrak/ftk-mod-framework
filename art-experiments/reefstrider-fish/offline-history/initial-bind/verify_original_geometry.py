...
import numpy as np
OUT=Path(__file__).resolve().parent;ROOT=OUT.parent.parent;target=ROOT/'scratch/reefstrider-original-proof';target.mkdir(exist_ok=True)
...
files=[]
for name in ['reefstrider.source.json','reefstrider_basecolor.png','reefstrider.pieces.json']:
