"""Create editable armature scenes, independently re-export, and render original Cinderbloom."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/cinderbloom-roundtrip-v2';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'cinderbloom_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Cinderbloom original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(120953,'cinderbloom_body'),(121072,'cinderbloom_leaves')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Cinderbloom surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'cinderbloom_body.blend'))
with bpy.data.libraries.load(str(OUT/'cinderbloom_leaves.blend'),link=False) as (source,target):target.objects=source.objects
...
light('Warm key',(2.8,-3.6,5),420,(1,.79,.57),4);light('Cool fill',(-3,-1,2),240,(.59,.78,1),3);light('Copper rim',(0,3,3),520,(1,.43,.18),2)
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=2.10
...
cam.location=(2.4,-4.8,2.5);aim(cam,(0,0,.73));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-3.5,-1.5,2.1);aim(cam,(0,0,.73));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(2.4,-4.8,2.5);aim(cam,(0,0,.73));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cinderbloom-studio.blend'))
# Separate still sequence keeps the original meshes in bind pose while the camera orbits.
frames=SCRATCH/'turntable';frames.mkdir(exist_ok=True);scene.render.resolution_x=720;scene.render.resolution_y=720;scene.cycles.samples=24
for frame in range(48):
 angle=2*math.pi*frame/48;cam.location=(4.8*math.sin(angle),-4.8*math.cos(angle),2.35);aim(cam,(0,0,.73));scene.render.filepath=str(frames/f'{frame:04d}.png');bpy.ops.render.render(write_still=True)
