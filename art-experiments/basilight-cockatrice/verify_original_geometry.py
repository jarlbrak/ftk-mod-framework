...
import numpy as np
OUT=Path(__file__).resolve().parent;ROOT=OUT.parent.parent;target=ROOT/'scratch/basilight-original-proof';target.mkdir(exist_ok=True)
...
files=[]
for name in ['basilight.source.json','basilight_basecolor.png','basilight.pieces.json']:
