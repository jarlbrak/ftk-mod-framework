...
def observation(frame=1,phase=None,done=True,ready=False):
    ...
    result.update(story=story,positionEligible=True,discoveryEligible=done,
                  atTargetHex=True,expectedQuestRegistered=True,
                  expectedQuestDefinition=True,expectedQuestDestination=True,ticket=None,
                  currentQuestId=1,currentQuestDefinition='ftkmf_hollowmire_crypt')
    if phase:
...
class EntryTests(unittest.TestCase):
    ...
    def test_visit_story_that_opens_after_initial_quiet_window_is_serviced_once(self):
        visit=observation(done=False);visit.update(positionEligible=False,discoveryEligible=False,
            expectedQuestDefinition=False,expectedQuestDestination=False,
            currentQuestId=-1,currentQuestDefinition='ftkmf_hollowmire_arrive')
        visit_done=observation(2);visit_done.update(positionEligible=False,discoveryEligible=False,
            expectedQuestDefinition=False,expectedQuestDestination=False,
            currentQuestId=-1,currentQuestDefinition='ftkmf_hollowmire_arrive')
        states=[visit,visit_done,observation(3),observation(4,'position-submitted'),
                observation(5,'discover-submitted',ready=True),observation(6,'discover-submitted',ready=True)]
        runner=Runner(states);prepared=self.run_prepare(runner);entry.verify(runner,prepared)
        self.assertEqual([c[0] for c in runner.calls].count('story-submit'),1)

    def test_visit_story_page_already_submitted_by_initial_clear_is_not_replayed(self):
        visit=observation(done=False);visit.update(positionEligible=False,discoveryEligible=False,
            expectedQuestDefinition=False,expectedQuestDestination=False,
            currentQuestId=-1,currentQuestDefinition='ftkmf_hollowmire_arrive')
        runner=Runner([visit]);runner.story_page_keys={entry.story.page_key(entry.story.submission(visit['story']))}
        with self.assertRaises(RuntimeError):self.run_prepare(runner)
        self.assertNotIn('story-submit',[c[0] for c in runner.calls])
    ...
    def test_initial_ineligible_or_existing_ticket_no_mutation(self):
        for change in [{'expectedQuestRegistered':False},{'ticket':{}},{'heroInstanceId':0}]:
