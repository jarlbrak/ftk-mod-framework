"""Prove the Sunspire Roc generator uses binding metadata, not native surfaces."""
import hashlib
import json
import sys
...
target = ROOT / 'scratch/sunspire-roc-original-proof'
...
try:
    ...
    # The independent export validation reads comparison metadata.  Stub it only
    # while proving authored source regeneration, then retain the real audit.
    ...
    code = (OUT / 'build_geometry.py').read_text().replace(
        'OUT=Path(__file__).resolve().parent;ROOT=OUT.parent.parent',
        f'OUT=Path({str(target)!r});ROOT=Path({str(ROOT)!r})',
    )
...
files = []
for name in ('sunspire-roc.source.json', 'sunspire-roc_basecolor.png', 'sunspire-roc.pieces.json'):
...
result = {
    'status': 'PASS',
    'method': 'Reran the exact authoring generator with output redirected and native NPZ reads limited to bone_names and bindposes. Export/validator were stubbed only for this source-generation proof.',
    'binding_keys_accessed': sorted(set(access)),
    'files': files,
    'generator_sha256': hashlib.sha256((OUT / 'build_geometry.py').read_bytes()).hexdigest(),
    'scope': 'Original-provenance proof only. The real GLB validation, Blender round trip, and live animation remain separate gates.',
}
