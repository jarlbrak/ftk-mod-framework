...
import numpy as np
OUT=Path(__file__).resolve().parent;ROOT=OUT.parent.parent;target=ROOT/'scratch/moonreed-original-proof';target.mkdir(exist_ok=True)
...
files=[]
for name in ['moonreed.source.json','moonreed_basecolor.png','moonreed.pieces.json']:
