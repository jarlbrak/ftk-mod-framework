#!/usr/bin/env python3
"""Review and, only with --execute, deploy staged original Kraken profiles.

The only allowed target is the disposable `scratch/mirewarden-game` copy. A
review validates every source and candidate hash without changing that copy.
An execution saves the prior catalog and relevant session/log artifacts in a
new timestamped backup before copying the candidate catalog and six assets.
"""
...
assert receipt["status"] == "STAGED_NOT_DEPLOYED_ABYSSAL_KRAKEN_ORIGINALS"
...
for name, digest in receipt["newAssets"].items():
    assert sha(staged_models / name) == digest, name
    assert not (models / name).exists(), name

old = read(catalog)
new = read(staged_catalog)
profiles = receipt["profiles"]
assert len(old["profiles"]) == receipt["profileRows"]["source"]
assert len(new["profiles"]) == receipt["profileRows"]["candidate"] == len(old["profiles"]) + len(profiles)
assert new["profiles"][:len(old["profiles"])] == old["profiles"]
assert new["profiles"][len(old["profiles"]):] == profiles
assert {key: value for key, value in new.items() if key != "profiles"} == {key: value for key, value in old.items() if key != "profiles"}
assert [profile["key"] for profile in profiles] == list(receipt["calibrations"])
for profile in profiles:
    calibration = receipt["calibrations"][profile["key"]]
    assert profile["baseEnemy"] == calibration["baseEnemy"]
    assert profile["combatProfile"] == calibration["combatProfile"]
    assert profile["renderers"][0]["rendererPath"] == calibration["rendererPath"]
...
review = {
    ...
    "profileKeys": [profile["key"] for profile in profiles],
    "changes": expected,
    "preservedProfileRows": len(old["profiles"]),
...
assert_game_stopped(GAME)
backup = GAME / "deployment-backups" / ("abyssal-kraken-" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
...
record = {
    "status": "IN_PROGRESS",
    "root": str(GAME),
    "stageReceipt": str(STAGE / "receipt.json"),
    "old": old_pins,
    "intendedChanges": expected,
    "completedCopies": [],
    "review": review,
}
