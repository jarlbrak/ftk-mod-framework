...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/mossglass-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 for slot in range(2):
  image=bpy.data.images.load(str(OUT/f'mossglass.slot{slot}.png'),check_existing=True);image.pack()
 ...
 return obj
for rid,name in [(121012,'mossglass')]:
 ...
 bpy.ops.wm.open_mainfile(filepath=str(OUT/f'{name}.blend'));export(OUT/'mossglass-reopened.glb',native_material_slots=2)
 validation=subprocess.run([str(PYTHON),str(ROOT/'tools/ai-model-pipeline/validate_glb.py'),str(OUT/'mossglass-reopened.glb'),'--reference',str(ref/'reference.npz')],capture_output=True,text=True,check=True)
 (OUT/'mossglass-reopened.validation.json').write_text(validation.stdout)
# Present the single original rigged source in a separate studio scene.
bpy.ops.wm.open_mainfile(filepath=str(OUT/'mossglass.blend'))
for obj in bpy.context.scene.objects:
 if obj.type=='ARMATURE':obj.hide_render=True;obj.hide_set(True)
scene=bpy.context.scene;scene.render.fps=12;scene['ftk_art_status']='Studio presentation: export original from mossglass.blend'
...
cam.location=(4,-7,4);aim(cam,(0,0,1.02));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'mossglass-studio.blend'))
