...
def validate_mixer(report,request,native,checks,label):
    require(set(request)=={'id','session','op','scenario','modernInputMixer'} and request['modernInputMixer'] in (MODE,FIXED_MODE),'Exact separate mixer request required')
    mode=request['modernInputMixer'];fixed=mode==FIXED_MODE
    ...
    for index,frame in enumerate(report['frames']):
        ...
        if fixed:
            ...
            names=[item['clip'] for item in inputs];require(len(set(names))==len(names),'Fixed graph cannot represent same-clip active roles')
            slot_weights=[0.]*4;slot_times=[0.]*4
            for item,weight,seconds in zip(inputs,weights,times):
                slot=FIXED_CLIPS.index(item['clip']);require(item['clipInstanceId']==fixed_ids[slot],'Active native clip differs from fixed slot')
        ...
        require(sample.get('graphMode')=='Manual' and sample.get('playableCount')==(5 if fixed else 3),'Wrong sampler graph shape')
...
def compare(first,request,repeat,repeat_request,native):
    ...
    if request.get('modernInputMixer')==FIXED_MODE:require(first['modernInputMixer']['identity']['fixedClipInstanceIds']==repeat['modernInputMixer']['identity']['fixedClipInstanceIds'],'Native fixed clip assets changed across repeat')
