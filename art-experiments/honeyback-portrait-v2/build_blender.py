"""Create editable armature scenes, independently re-export, and render original Honeyback portrait v2."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/honeyback-portrait-v2-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'honeyback_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Honeyback portrait v2 original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121467,'honeyback_portrait_v2')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Honeyback portrait v2 surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'honeyback_portrait_v2.blend'))
...
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=4.35
...
cam.location=(4.2,-5.7,3.2);aim(cam,(0,-.80,1.12));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-5.8,-1.4,3.0);aim(cam,(0,-.80,1.12));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(4.2,-5.7,3.2);aim(cam,(0,-.80,1.12));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'honeyback-portrait-v2-studio.blend'))
