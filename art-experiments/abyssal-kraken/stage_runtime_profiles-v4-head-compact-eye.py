#!/usr/bin/env python3
"""Stage the V4 compact-eye correction without changing the game.

The baseline contains the rejected V3 composite and the separately archived V3
Sea King correction. This stage changes only the modern Kraken head row to use
one smaller static eye asset. It retains every prior asset in the game copy.
"""
...
BASELINE = "runtime-profiles-v3-head-static-eye.json"
CANDIDATE = "runtime-profiles.json"
CHANGED_KEY = "ftkmf_modeltest_abyssal_crown_kraken_head"
NEW_ASSETS = {"abyssal-crown-kraken-eye-v4.glb", "abyssal-crown-kraken-eye-v4.png"}
RETIRED_PROFILE_ASSETS = {"abyssal-crown-kraken-eye-v3.glb", "abyssal-crown-kraken-eye-v3.png"}
...
def expected_head_v4_row(old: dict) -> dict:
    assert old["displayName"] == "Abyssal Crown V3"
...
assert changes[0][1] == expected_head_v4_row(changes[0][0]), "Unexpected V4 compact-eye correction."
...
for old, new in zip(old_profiles, new_profiles):
    ...
    index = matches[0]
    assert rows[index] == old, f"Current row drifted from the V3 head baseline: {old['key']}"
...
for name in (
    "runtime-profiles-v1.json", "runtime-profiles-v2.json", "runtime-profiles-v2.1.json", "runtime-profiles-v3-seaking.json", BASELINE, CANDIDATE,
    ...
    "stage_runtime_profiles-v3-seaking.py", "deploy_runtime_profiles-v3-seaking.py", "stage_runtime_profiles-v3-head-static-eye.py", "deploy_runtime_profiles-v3-head-static-eye.py",
):
...
receipt = {
    "status": "STAGED_NOT_DEPLOYED_ABYSSAL_KRAKEN_V4_HEAD_COMPACT_EYE",
