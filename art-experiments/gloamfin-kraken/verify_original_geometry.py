...
import numpy as np
OUT=Path(__file__).resolve().parent;ROOT=OUT.parent.parent;target=ROOT/'scratch/gloamfin-original-proof';target.mkdir(exist_ok=True)
...
files=[]
for name in ['gloamfin.source.json','gloamfin_basecolor.png','gloamfin.pieces.json']:
