#!/usr/bin/env python3
"""Stage a hash-pinned V2.1 Sea King emission correction.

V1 and V2 assets and evidence remain untouched. This changes only the two
existing Sea King catalog rows, opting their already-installed source texture
out of inherited matLoot emission. The script reads the disposable game copy
and creates a new immutable stage under scratch; it never changes the game.
"""
...
BASELINE = "runtime-profiles-v2.json"
...
def expected_v21_row(old: dict) -> dict:
    new = copy.deepcopy(old)
    new["displayName"] = new["displayName"].replace("V2 ", "V2.1 ")
    new["renderers"][0]["disableNativeEmission"] = True
    return new
...
assert asset_names(old_profiles) == asset_names(new_profiles), "V2.1 must reuse the pinned V2 asset identities."
changes = [(old, new) for old, new in zip(old_profiles, new_profiles) if old != new]
assert [old["key"] for old, _ in changes] == list(CHANGED_KEYS), "V2.1 may change only the measured Sea King material rows."
assert all(new == expected_v21_row(old) for old, new in changes), "Unexpected V2.1 row change."
...
for old, new in zip(old_profiles, new_profiles):
    ...
    index = matches[0]
    assert rows[index] == old, f"Current row drifted from retained V2 baseline: {old['key']}"
...
assert all(name in prior_assets for name in asset_names(old_profiles)), "Pinned V2 assets are missing from the isolated game copy."
...
for name in (
    "runtime-profiles-v1.json", BASELINE, CANDIDATE,
...
receipt = {
    "status": "STAGED_NOT_DEPLOYED_ABYSSAL_KRAKEN_V2_1_EMISSION_CORRECTION",
    ...
    "pinnedV2Assets": {name: prior_assets[name] for name in sorted(asset_names(old_profiles))},
    "authoringFiles": {name: sha(ART / name) for name in ("runtime-profiles-v1.json", BASELINE, CANDIDATE, "build_geometry.py", "build_blender.py", "build-report.json", "original-geometry-proof.json", "target-bindings-proof.json", "verify_original_geometry.py", "verify_target_bindings.py")},
    "stageScriptSha256": sha(OUT / "stage-script.py"),
    "limits": "Correction-stage proof only. V1 and V2 evidence are retained. V2.1 requires fresh registration, exact owner binding, material observation, animation, gameplay, death, Ready, culling, and visual review.",
}
