...
import numpy as np
OUT=Path(__file__).resolve().parent;ROOT=OUT.parent.parent;target=ROOT/'scratch/mossglass-original-proof';target.mkdir(exist_ok=True)
...
try:
 ...
 code=(OUT/'build_geometry.py').read_text().split('# AUTHORING_COMPLETE:')[0].replace('OUT=Path(__file__).resolve().parent;ROOT=OUT.parent.parent',f'OUT=Path({str(target)!r});ROOT=Path({str(ROOT)!r})')
...
files=[]
for name in ['mossglass.source.json','mossglass.slot0.png','mossglass.slot1.png','mossglass.pieces.json']:
