#!/usr/bin/env python3
"""Stage a V1 static-eye placement probe without changing the game.

The baseline contains the V5 reattached-eye trial. This stage changes only the
modern Kraken head row to use six original colored calibration markers. It
retains every prior asset and exists only for isolated placement evidence.
"""
...
BASELINE = "runtime-profiles-v5-head-reattached-eye.json"
CANDIDATE = "runtime-profiles-placement-probe-v1.json"
CHANGED_KEY = "ftkmf_modeltest_abyssal_crown_kraken_head"
NEW_ASSETS = {"abyssal-crown-kraken-eye-placement-probe-v1.glb", "abyssal-crown-kraken-eye-placement-probe-v1.png"}
RETIRED_PROFILE_ASSETS = {"abyssal-crown-kraken-eye-v5.glb", "abyssal-crown-kraken-eye-v5.png"}
...
def expected_placement_probe_row(old: dict) -> dict:
    assert old["displayName"] == "Abyssal Crown V5"
    ...
    new["displayName"] = "Abyssal Crown Placement Probe V1"
...
assert changes[0][1] == expected_placement_probe_row(changes[0][0]), "Unexpected static-eye placement probe."
...
for old, new in zip(old_profiles, new_profiles):
    ...
    index = matches[0]
    assert rows[index] == old, f"Current row drifted from the V5 head baseline: {old['key']}"
...
for name in (
    "runtime-profiles-v1.json", "runtime-profiles-v2.json", "runtime-profiles-v2.1.json", "runtime-profiles-v3-seaking.json", "runtime-profiles-v3-head-static-eye.json", "runtime-profiles-v4-head-compact-eye.json", BASELINE, CANDIDATE,
...
receipt = {
    "status": "STAGED_NOT_DEPLOYED_ABYSSAL_KRAKEN_STATIC_EYE_PLACEMENT_PROBE_V1",
