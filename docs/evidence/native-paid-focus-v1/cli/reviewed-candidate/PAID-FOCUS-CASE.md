...
Standalone offline-reviewed source candidate; no shared runner/helper modification. Native manual helper success is separate from CLI live acceptance. This wrapper spends at most one focus slot and never attacks, selects another action, refunds, dismisses a modal, advances a turn, or invokes a fallback.

```sh
python3 scratch/paid-focus-runner-candidate/paid_focus_case.py \
