...
## Revised connected mantle and faithful material division

The rejected block-lobe generation, exact hero/side and hashes are preserved in offline-history/rejected-block-mantle-v1. The current mantle is one continuous capped surface from narrow hem through waist/chest, using the native hip/knee blends at its lower end. Sleeves taper into the wrists. This replaces the detached-boot reading; it still needs native moving-cloth review.

Current direct and reopened exports use two genuinely disjoint primitives. Slot0 contains cloth with a dedicated repeat-safe blue-green tile; slot1 contains fixed mask/hand palette surfaces. No triangle is duplicated or omitted. material-partition-audit.json records the split. Native ScrollingUVs slot0/_MainTex rate(0,-.5) remains a runtime behavior to preserve and verify, not baked author animation. No palette atlas feeds the scrolling cloth. Source findings18ea071d pin native scale1.5, separate material properties, eight particle systems, FlickerLight/Light and no rigidbody/collider ragdoll. Runtime emission options and exact material readback still need integration and live validation. No staging or frozen art acceptance.
