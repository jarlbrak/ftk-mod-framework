...
import numpy as np
OUT=Path(__file__).resolve().parent;ROOT=OUT.parent.parent;target=ROOT/'scratch/amberwake-original-proof';target.mkdir(exist_ok=True)
...
files=[]
for name in ['amberwake.source.json','amberwake_basecolor.png','amberwake.pieces.json']:
