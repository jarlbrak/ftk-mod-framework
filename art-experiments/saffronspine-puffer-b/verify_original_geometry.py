...
import numpy as np
OUT=Path(__file__).resolve().parent;ROOT=OUT.parent.parent;target=ROOT/'scratch/saffronspine_b-original-proof';target.mkdir(exist_ok=True)
...
files=[]
for name in ['saffronspine_b.source.json','saffronspine_b_basecolor.png','saffronspine_b.pieces.json']:
