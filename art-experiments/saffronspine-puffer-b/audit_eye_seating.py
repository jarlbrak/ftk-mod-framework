...
import numpy as np
O=Path(__file__).resolve().parent;R=O.parent.parent;d=json.loads((O/'saffronspine_b.source.json').read_text());parts=json.loads((O/'saffronspine_b.pieces.json').read_text());v=np.c_[d['positions'],np.ones(len(d['positions']))];j=np.array(d['joints']);w=np.array(d['weights']);ref=np.load(R/'scratch/puffer-b-native-topology-analysis/reference-121388/reference.npz');body=parts[0];tri=np.array(d['triangles'])[:body['vertex_count']//3];eyes=[]
for q in parts:
...
reports=[]
for name,cid in [('pass','734cd8b8c3a147d0bfc724b570f28ea6'),('hit','aaa996dca2d242e6bc4067a48654e954'),('death','d94a606c383e4ec5b6437396bd8216a9')]:
