"""Create editable armature scenes, independently re-export, and render original Emberglass Bee."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/emberglass-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'emberglass_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Emberglass Bee original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121062,'emberglass')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Emberglass Bee surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'emberglass.blend'))
for obj in bpy.context.scene.objects:
 if obj.type=='ARMATURE':obj.hide_render=True;obj.hide_set(True)
# Studio source is rotated as a single rigid presentation; bind source scene/export stays untouched.
for obj in bpy.context.scene.objects:
 if obj.type in ['ARMATURE','MESH'] and obj.parent is None:obj.rotation_euler.x=-math.pi/2
...
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=1.7
...
cam.location=(1.1,-1.8,1.15);aim(cam,(0,.1,.34));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-1.7,-.6,.95);aim(cam,(0,.1,.34));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(1.1,-1.8,1.15);aim(cam,(0,.1,.34));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'emberglass-studio.blend'))
