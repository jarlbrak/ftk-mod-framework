...
for(const p of catalog.packages){const r=await fetch(p.screenshots[0]);if(!r.ok)throw Error(r.status);await convert(Buffer.from(await r.arrayBuffer()),p.name.toLowerCase(),p.screenshots[0],p.name==='Paladin'?'native game capture':'promotional artwork');}
