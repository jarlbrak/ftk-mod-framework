#!/usr/bin/env python3
"""Build deterministic Thief artifacts without publishing them."""
import argparse
import hashlib
import io
import json
...
def main():
    ...
    parser.add_argument('--output',type=Path,default=ROOT/'scratch/thief-package-build')
    parser.add_argument('--game-assembly',type=Path,default=ROOT/'scratch/thief-game-ui-retest/FTKPlaytest.app/Contents/Resources/Data/Managed/Assembly-CSharp.dll')
    ...
    subprocess.run([sys.executable,str(HERE/'validate_thief.py')],check=True)
    ...
    stem='thief-'+manifest['version']+'-'+sha[:12]
    tag='thief-v'+manifest['version'] if args.release else 'LOCAL-DRAFT-NOT-PUBLISHED'
    release_url='https://github.com/jarlbrak/ftk-mod-framework/releases/download/'+tag+'/'
    preview_url=release_url+'thief-playtest-banner.png'
    banner_bytes=(PACKAGE/'promo/thief-playtest-banner.png').read_bytes()
    if len(banner_bytes)>2*1024*1024 or banner_bytes[:8]!=b'\x89PNG\r\n\x1a\n':
        raise ValueError('Banner must be a PNG no larger than 2 MiB')
    width,height=struct.unpack('>II',banner_bytes[16:24])
    if not (0<width<=4096 and 0<height<=4096 and width*height<=8388608):
        raise ValueError('Banner exceeds marketplace dimensions')
    immutable(output/'thief-playtest-banner.png',banner_bytes)
    if args.preview_state_root:
        ...
        banner=PACKAGE/'promo/thief-playtest-banner.png'
    ...
    receipt={'archive':archive_path.name,'descriptor':descriptor_path.name,'archiveSha256':sha,'descriptorSha256':digest(descriptor_bytes),
             'bannerSha256':digest(banner_bytes),'bannerDimensions':[width,height],
