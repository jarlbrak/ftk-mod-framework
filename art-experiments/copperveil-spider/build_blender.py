"""Create editable armature scenes, independently re-export, and render original Copperveil Weaver."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/copperveil-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'copperveil_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Copperveil Weaver original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121386,'copperveil')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Copperveil Weaver surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'copperveil.blend'))
...
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=3.95
...
cam.location=(3.1,-5.2,3.5);aim(cam,(0,.10,.57));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-4.8,-1.8,2.8);aim(cam,(0,.10,.57));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(3.1,-5.2,3.5);aim(cam,(0,.10,.57));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'copperveil-studio.blend'))
