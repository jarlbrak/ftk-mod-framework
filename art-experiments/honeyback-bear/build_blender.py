"""Create editable armature scenes, independently re-export, and render original Honeyback Bear."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/honeyback-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 material=bpy.data.materials.new('Honeyback Bear original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121467,'honeyback')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Honeyback Bear surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'honeyback.blend'))
...
cam.location=(4.2,-5.7,3.2);aim(cam,(0,-.80,1.12));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'honeyback-studio.blend'))
