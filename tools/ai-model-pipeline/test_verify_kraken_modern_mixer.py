...
from verify_kraken_modern_mixer import compare,validate_mixer,Checks,MODE,FIXED_MODE,FIXED_CLIPS,run
...
def fixed_fixture(case='a',start=1,appearance=False):
 r,q,n=fixture(case,start,appearance);q['modernInputMixer']=FIXED_MODE;r['modernInputMixer']['mode']=FIXED_MODE
 meta=r['modernInputMixer']['identity'];meta['mode']=FIXED_MODE
 ids=dict(zip(FIXED_CLIPS,[77,79,80,81]))
 for f in r['frames']:
  for item in f['modernInputMixer']['inputs']:ids[item['clip']]=item['clipInstanceId']
 fixed_ids=[ids[name] for name in FIXED_CLIPS]
 meta.update(fixedClipOrder=FIXED_CLIPS[:],fixedClipInstanceIds=fixed_ids[:],bindingInitialization='All four clip nodes connected before Animator.Rebind; fixed topology throughout run.')
 for f in r['frames']:
  sample=f['modernInputMixer'];weights=[0.]*4;times=[0.]*4
  for item in sample['inputs']:
   slot=FIXED_CLIPS.index(item['clip']);weights[slot]=item['rawWeight'];times[slot]=item['sampleSeconds']
  sample.update(mode=FIXED_MODE,playableCount=5,inputWeights=weights,playableTimes=times,fixedClipInstanceIds=fixed_ids[:])
 return r,q,n


class FixedTests(unittest.TestCase):
 def bad(self,change):
  r,q,n=fixed_fixture();change(r);c=Checks()
  try:validate_mixer(r,q,n,c,'fixed')
  except (ValueError,KeyError):return
  self.assertTrue(c.failures,'Corrupted fixed graph report passed')
 def test_main_and_appearance_fixed_graph_pairs(self):
  for appearance in (False,True):
   a,aq,n=fixed_fixture(appearance=appearance);b,bq,_=fixed_fixture('b',500,appearance)
   self.assertEqual(compare(a,aq,b,bq,n)['status'],'modern_input_mixer_match')
 def test_inactive_slot_clock_weight_and_identity_rejected(self):
  self.bad(lambda r:r['frames'][0]['modernInputMixer']['playableTimes'].__setitem__(2,.1))
  self.bad(lambda r:r['frames'][0]['modernInputMixer']['inputWeights'].__setitem__(2,.1))
  self.bad(lambda r:r['frames'][0]['modernInputMixer']['fixedClipInstanceIds'].__setitem__(2,999))
  self.bad(lambda r:r['modernInputMixer']['identity']['fixedClipOrder'].reverse())
  self.bad(lambda r:r['frames'][0]['modernInputMixer'].update(playableCount=3))
 def test_same_clip_roles_refused_without_aggregation(self):
  def corrupt(r):
   f=r['frames'][18];s=f['modernInputMixer'];weight=s['inputs'][1]['rawWeight'];first=s['inputs'][0]
   f['modern']['next']=copy.deepcopy(f['modern']['current']);f['modern']['nextClips']=copy.deepcopy(f['modern']['currentClips']);f['modern']['nextClips'][0]['weight']=weight
   s['inputs'][1]=copy.deepcopy(first);s['inputs'][1].update(role='next',rawWeight=weight)
  self.bad(corrupt)
 def test_zero_weight_appearance_stays_main_eligible(self):
  a,aq,n=fixed_fixture(appearance=True);b,bq,_=fixed_fixture('b',500,True)
  for r in (a,b):
   f=next(f for f in r['frames'] if f['modern']['inTransition']);s=f['modernInputMixer']
   for side in ('modern','old'):f[side]['currentClips'][0]['weight']=1.;f[side]['nextClips'][0]['weight']=0.
   s['inputs'][0]['rawWeight']=1.;s['inputs'][1]['rawWeight']=0.;s['inputWeights']=[1.,0.,0.,0.]
   s.update(eligibleMainComparison=True,appearanceZeroWeightBoundary=True,scope='main_whole_controller_comparison_including_history',mainMaximumError=0.,mainComparisonPassed=True)
   r['modernInputMixer']['eligibleFrames']+=1;r['modernInputMixer']['excludedAppearanceFrames']-=1
  result=compare(a,aq,b,bq,n);self.assertEqual(result['status'],'modern_input_mixer_match');self.assertIn(f['step'],result['firstCoverage']['eligibleSteps'])
 def test_modes_cannot_be_paired(self):
  a,aq,n=fixed_fixture();b,bq,_=fixture('b',500)
  with self.assertRaises(ValueError):compare(a,aq,b,bq,n)
