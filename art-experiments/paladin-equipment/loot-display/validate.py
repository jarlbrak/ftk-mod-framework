...
for record in manifest['assets']:
 ...
 results.append({'key':KEY,'vertices':len(p),'triangles':len(t),'status':'PASS'})
assert len(results)==6
report={'status':'PASS','scope':'Independent binary/source equality, finite arrays, indices, UVs, normals, winding and index bounds. No native card fit or runtime claim.','assets':results}
(OUT/'validation.json').write_text(json.dumps(report,indent=2)+'\n')
print('PASS: 6 original static loot display GLBs')
