#!/usr/bin/env python3
"""Author the V5 reattached rigid companion for the modern Kraken eye renderer.
...
ASSET = "abyssal-crown-kraken-eye-v5"
TEXTURE = ASSET + ".png"
GLB = OUT / (ASSET + ".glb")
...
REFERENCE = ROOT / "scratch/skeleton-audit/121035/reference.npz"
REQUIRED_BONES = ["Root_M", "base", "body", "neck", "head", "topHead", "eye"]


def derive_head_core_anchor() -> tuple[np.ndarray, dict[str, object]]:
    """Place the rigid jewel at the V2 head's own authored crown core.

    This reads only the permitted production palette names and inverse bind
    matrices. It never opens or derives anything from the native static eye
    mesh. The resulting coordinate is in the selected MeshFilter's local
    space, which is the eye transform's local space at bind pose.
    """
    reference = np.load(REFERENCE, allow_pickle=False)
    names = reference["bone_names"].tolist()
    assert names == REQUIRED_BONES, "Modern Kraken palette changed"
    inverse_bind = reference["bindposes"]
    bind_centers = np.linalg.inv(inverse_bind)[:, :3, 3]
    center_by_bone = dict(zip(names, bind_centers))
    # This is the same original core landmark used by build_geometry.py for
    # the V2 skinned head, not a measurement of the game-owned eye surface.
    own_head_core = center_by_bone["head"] + np.array([0.0, .10, .04])
    eye_index = names.index("eye")
    anchor_h = inverse_bind[eye_index] @ np.r_[own_head_core, 1.0]
    assert np.isfinite(anchor_h).all() and abs(float(anchor_h[3]) - 1.0) < 1e-6
    anchor = anchor_h[:3]
    return anchor, {
        "reference": str(REFERENCE.relative_to(ROOT)),
        "referenceBoneNames": names,
        "ownHeadCoreMeshSpace": own_head_core.tolist(),
        "eyeLocalAnchor": anchor.tolist(),
        "derivation": "eye inverse bind matrix multiplied by the original V2 head-core landmark",
        "boundary": "Uses permitted palette metadata and an earlier original mesh landmark only. No native static mesh surface, bounds, texture, UVs, weights, or animation data are read.",
    }
...
def validate_static_glb() -> dict:
    ...
    return {
        "status": "PASS_ABYSSAL_KRAKEN_STATIC_EYE_V5_CONTRACT",
        "asset": ASSET,
...
def main() -> None:
    ...
    at = lambda offset: anchor + np.asarray(offset, dtype=float)
    # V3 proved that a complete globe overwhelms the skinned head in the live
    # combat camera. V4 established a compact jewel, while its fresh capture
    # showed that the target pivot sits below the authored head. V5 places the
    # same jewel at the earlier original V2 head-core landmark in eye-local
    # space, never by consulting the native static mesh.
    ...
    surface.ellipsoid(
        "Reattached compressed crown-eye casing", anchor, [.72, .48, .25],
    ...
    for direction, label in ((1.0, "front"), (-1.0, "back")):
        surface.ellipsoid(
            f"{label} sea-glass lens", at([0, .01, direction * .255]), [.43, .30, .052],
            rings=6, segments=12, atlas=glass_atlas,
        )
        surface.torus(
            f"{label} copper bezel", at([0, .01, direction * .295]), .455, .052, direction,
            segments=14, tube=5, atlas=copper_atlas,
        )
        surface.ellipsoid(
            f"{label} vertical pupil", at([0, .01, direction * .355]), [.105, .185, .020],
    ...
    for index, x in enumerate((-.30, 0.0, .30), start=1):
        surface.ellipsoid(
            f"Crown cabochon {index}", at([x, .455 + (.055 if index == 2 else 0.0), 0]),
            [.075, .10, .050], rings=5, segments=8, atlas=crown_atlas,
        )
    data = surface.data()
    data["placementAnchor"] = anchor_info
    data["authoringBoundary"] = (
        "Original procedural rigid geometry. No native static mesh surface, bounds, texture pixels, UVs, skin weights, or animation data are read. "
        "The local placement anchor is derived only from permitted inverse bind matrices and the earlier original V2 head-core landmark."
    )
