...
import numpy as np
OUT=Path(__file__).resolve().parent;ROOT=OUT.parent.parent;target=ROOT/'scratch/tamarind-original-proof';target.mkdir(exist_ok=True)
...
files=[]
for name in ['tamarind.source.json','tamarind_basecolor.png','tamarind.pieces.json']:
