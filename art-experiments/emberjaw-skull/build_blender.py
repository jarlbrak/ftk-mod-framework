"""Create editable armature scenes, independently re-export, and render original Emberjaw."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/emberjaw-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 image=bpy.data.images.load(str(OUT/'emberjaw_basecolor.png'),check_existing=True);image.pack()
 material=bpy.data.materials.new('Emberjaw original palette');material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.78
 ...
 return obj
for rid,name in [(121577,'emberjaw_top'),(121483,'emberjaw_bottom')]:
 ...
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Emberjaw surface; live validation pending'
...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'emberjaw_top.blend'))
with bpy.data.libraries.load(str(OUT/'emberjaw_bottom.blend'),link=False) as (source,target):target.objects=source.objects
...
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=3.70
...
cam.location=(2.6,-6,3.3);aim(cam,(0,0,1.75));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-4.8,-2.4,3.1);aim(cam,(0,0,1.75));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(2.6,-6,3.3);aim(cam,(0,0,1.75));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'emberjaw-studio.blend'))
