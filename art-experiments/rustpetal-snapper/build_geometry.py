"""Original parametric Rustpetal Snapper surfaces; native data supplies binding landmarks only."""
...
PALETTE=['3E4930','657348','87965B','ACB376','8B4330','B86339','D99053','E8D3A1','9D723E','DFAC64','172D37','BC8B4C']
image=Image.new('RGB',(384,32))
for i,h in enumerate(PALETTE):image.paste(tuple(int(h[k:k+2],16) for k in (0,2,4)),(i*32,0,(i+1)*32,32))
image.save(OUT/'rustpetal_basecolor.png')
class Surface:
 def __init__(self,rid):
  self.rid=rid;self.ref=np.load(ROOT/'scratch/plant-d-native-topology-analysis/reference-121537/reference.npz',allow_pickle=False);self.names=self.ref['bone_names'].tolist();self.centers=np.linalg.inv(self.ref['bindposes'])[:,:3,3];self.B=dict(zip(self.names,self.centers));self.data={k:[] for k in ['positions','normals','uvs','triangles','joints','weights']};self.data['bone_names']=self.names;self.pieces=[]
...
b.ellipsoid('Broad rust seedpod hood',[0,1.22,-.04],[.36,.135,.15],'Head',[4,5,6,5,4,5,6,5,4,5])
...
front=[];back=[];N=24
for k in range(N):
 t=2*math.pi*k/N;front.append(np.array([.34*math.cos(t),1.195+.10*math.sin(t),.318]));back.append(np.array([.10*math.cos(t),1.195+.05*math.sin(t),.115]))
...
for radiusx,radiusy,z in [(.28,.115,-.045),(.36,.135,.14),(.357,.12,.313)]:
...
b.pieces.append(dict(name='Continuous rust outer lip-to-pod flesh',vertex_start=start,vertex_count=len(b.data['positions'])-start,openSurface=True))
...
upper=[[.35*__import__('math').cos(t),1.195+.11*__import__('math').sin(t),.32]for t in np.linspace(0,math.pi,9)]
lower=[[.35*__import__('math').cos(t),1.195+.11*__import__('math').sin(t),.32]for t in np.linspace(math.pi,2*math.pi,9)]
...
for x in [-.26,-.16,-.07,.07,.16,.26]:
 y=1.195+.11*math.sqrt(1-(x/.35)**2);b.tube('Pale upper tooth '+str(x),[[x,y-.01,.334],[x,y-.073,.345]],[.022,.003],[.021,.003],['Head']*2,7,sides=5)
...
# Two broad split petal blades form the distinct seedpod silhouette, anchored to Head.
for sign in [-1,1]:
 b.tube('Split upper rust petal '+str(sign),[[sign*.14,1.30,-.035],[sign*.29,1.46,-.08],[sign*.37,1.66,-.15]],[.085,.14,.007],[.037,.029,.005],['Head']*3,[4,5,6,7,6,5],axis=(1,0,0),sides=6)
b.tube('Broad opposite side leaf Leaf5',[b.B['Bone007'],b.B['Leaf5']+[.04,.01,.04],b.B['Leaf5']+[.27,.12,.04]],[.035,.14,.006],[.027,.033,.005],['Bone007','Leaf5','Leaf5'],[0,1,2,3,2,1],axis=(0,1,0),sides=6)
b.write('rustpetal')
