"""Create editable armature scenes, independently re-export, and render original Mossglass Reliquary."""
...
PYTHON=ROOT/'scratch/model-venv/bin/python';SCRATCH=ROOT/'scratch/mossglass_caps_v3-roundtrip';SCRATCH.mkdir(parents=True,exist_ok=True)
def import_original(name,arm):
 ...
 for slot in range(2):
  image=bpy.data.images.load(str(OUT.parent/'mossglass.slot0.png' if slot==0 else OUT.parent/'jade-v2/mossglass_jade_v2.slot1.png'),check_existing=True);image.pack()
  material=bpy.data.materials.new('Original native slot '+str(slot));material.use_nodes=True;nodes=material.node_tree.nodes;shader=nodes.get('Principled BSDF');shader.inputs['Roughness'].default_value=.64
 ...
 groups={tuple(t):slot for slot,p in enumerate(d['primitives']) for t in p['triangles']}
 for p in mesh.polygons:p.material_index=groups[tuple(p.vertices)]

 return obj
for rid,name in [(121012,'mossglass_caps_v3')]:
 ref=ROOT/'scratch/cube-topology-analysis/reference-121012';create(ref/'reference.npz',ref/'skeleton.json',SCRATCH/f'{name}-template.blend',PYTHON,False)
 arm=next(o for o in bpy.data.objects if o.type=='ARMATURE')
 for obj in list(bpy.data.objects):
  if obj.type=='MESH':bpy.data.objects.remove(obj,do_unlink=True)
 obj=import_original(name,arm);bpy.context.scene['ftk_art_status']='Original Mossglass Reliquary surface; live validation pending'
 ...
 bpy.ops.wm.open_mainfile(filepath=str(OUT/f'{name}.blend'));export(OUT/'mossglass_caps_v3-reopened.glb',native_material_slots=2)
 validation=subprocess.run([str(PYTHON),str(ROOT/'tools/ai-model-pipeline/validate_glb.py'),str(OUT/'mossglass_caps_v3-reopened.glb'),'--reference',str(ref/'reference.npz')],capture_output=True,text=True,check=True)
 (OUT/'mossglass_caps_v3-reopened.validation.json').write_text(validation.stdout)
# Present the single original rigged source in a separate studio scene.
bpy.ops.wm.open_mainfile(filepath=str(OUT/'mossglass_caps_v3.blend'))
for obj in bpy.context.scene.objects:
 if obj.type=='ARMATURE':obj.hide_render=True;obj.hide_set(True)
for mat in bpy.data.materials:
 if mat.name.startswith('Original native slot 1'):
...
scene=bpy.context.scene;scene.render.fps=12;scene['ftk_art_status']='Studio presentation: export original from mossglass_caps_v3.blend'
...
def light(name,loc,power,color,size):
 data=bpy.data.lights.new(name,'AREA');data.energy=power;data.color=color;data.shape='DISK';data.size=size;obj=bpy.data.objects.new(name,data);scene.collection.objects.link(obj);obj.location=loc;aim(obj,(0,0,1))
light('Warm key',(4,-5,7),650,(.82,.90,1),4);light('Cool fill',(-4,-2,4),400,(.59,.78,1),3);light('Copper rim',(0,4,6),900,(.70,.62,1),2)
camdata=bpy.data.cameras.new('Studio camera');cam=bpy.data.objects.new('Studio camera',camdata);scene.collection.objects.link(cam);scene.camera=cam;camdata.type='ORTHO';camdata.ortho_scale=3.6
...
cam.location=(4,-7,4);aim(cam,(0,0,1.02));scene.render.filepath=str(OUT/'hero.png');bpy.ops.render.render(write_still=True)
cam.location=(-7,-1,3.2);aim(cam,(0,0,1.02));scene.render.filepath=str(OUT/'side.png');bpy.ops.render.render(write_still=True)
cam.location=(4,-7,4);aim(cam,(0,0,1.02));bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'mossglass_caps_v3-studio.blend'))
