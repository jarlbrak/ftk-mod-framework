#!/usr/bin/env python3
"""Archive the fresh Copperveil Spider B combat trial without game payloads."""
...
ASSET = ROOT / "art-experiments" / "copperveil-spider"
OUT = ASSET / "live-validation-v2"
BASE = ROOT / "scratch" / "mirewarden-game" / "model-test-output"
SESSION = "115d691fdeec4e829088462912641932"
CASE = BASE / "case-2b7d1b04ceed4c528ebc1d342fc68bd2" / "case-result.json"
JOURNAL = BASE / "case-2b7d1b04ceed4c528ebc1d342fc68bd2" / "journal.jsonl"
PROFILE = ROOT / "scratch" / "mirewarden-game" / "model-test-profiles.json"
REVIEW = ROOT / "scratch" / "copperveil-root-visual-review.json"
...
assert case["session"] == SESSION and case["enemy"] == "ftkmf_modeltest_copperveil"
...
assert review["binding"] == {
    "nativeChassis": "spiderB",
    "rendererPath": "enSpiderB",
    "rendererId": 121386,
    "ownerInstanceId": 369188,
    "boneSignature": "5ecc6ff62492505b0f95f19ce57e5f5b2fa6e04b01dc0156c56c0c6e78cbfb6e",
    "visualScale": 1.0,
}
...
for name in (
    ...
    "copperveil.source.json",
    "copperveil.validation.json",
    "copperveil.pieces.json",
...
# Preserve the fresh setup journal and result referenced by the new-run marker.
...
# Helper responses referenced by any retained journal are useful for replay audits.
...
write(
    OUT / "asset-pins.json",
    {
        ...
        for name in ("copperveil.glb", "copperveil_basecolor.png")
...
validation = {
    ...
    "session": SESSION,
    "enemy": "ftkmf_modeltest_copperveil",
    "displayName": "Copperveil Weaver",
    "nativeChassis": "spiderB",
    "rendererPath": "enSpiderB",
    "rendererId": 121386,
    "ownerInstanceId": 369188,
    ...
    "assetHashes": {
        "copperveil.glb": manifest["assets"]["copperveil.glb"]["sha256"],
        "copperveil_basecolor.png": manifest["assets"]["copperveil_basecolor.png"]["sha256"],
    },
    "captures": captures,
    "ordinaryHit": {"beforeHp": 63, "afterHp": 61, "cheat": "None", "focus": False},
    ...
    "progression": "One native Collect was accepted and strict native Ready was observed at level 0 room 2; no second Collect was submitted.",
...
(OUT / "README.md").write_text(
    """# Copperveil Weaver fresh live trial V2

This supplement records the fresh catalog-411 run in session `115d691fdeec4e829088462912641932` using Spider B, renderer `enSpiderB` (121386), owner `369188`, and visual scale `1.0`. The authored eight-legged surface bound to the exact native renderer and stayed readable across the pass, ordinary attack and explicit fixture captures.

The pass and attack are complete 120-frame recordings. The ordinary attack changed the same target from HP 63 to 61 with `cheat=None` and no focus. The explicit `KillSingle` fixture also completed 120 frames, removed the enemy, accepted one native Collect and reached strict native Ready at level 0 room 2. The fixture death is not ordinary lethal damage.

Root reviewed two settled idle frames, two poison-attack frames and the fixture endpoints. Body, copper abdomen markings, amber eyes, articulated legs and antennae remain readable; native poison effects and the hero foreground limit fine deformation and material inspection. The terminal view is the native victory/item-choice surface, so it does not establish detailed corpse or floor-contact acceptance.

`validation.json` preserves the case result, setup/action journals, helper responses, profile/runtime/authoring manifests as gzip-lossless metadata, all 360 source-image hashes, six selected originals and three 120-frame presentation videos. Native payloads and DLLs are excluded. `archive.py` is offline-only and refuses a completed destination.
"""
)
print(
    json.dumps(
