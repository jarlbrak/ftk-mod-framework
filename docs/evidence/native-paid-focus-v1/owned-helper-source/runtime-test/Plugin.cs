...
public sealed partial class RuntimeModelTest : BaseUnityPlugin
{
    ...
    void Awake()
    {
        ...
        File.WriteAllText(Path.Combine(root,"model-test-session.json"),new JObject{{"session",sessionId},{"savePath",isolatedSavePath}}.ToString());
    ...
    void Update()
    {
        ...
        try
        {
            ...
            else if(op == "paid-focus-state") Finish(id,PaidFocusState(command));
            else if(op == "paid-focus-submit") Finish(id,PaidFocusSubmit(command));
            ...
            else if (op == "capture" || op == "play")
            {
                ...
                busy = true;
                StartCoroutine(Capture(id, renderer, seconds, fps, maxWidth, fixedStep, op == "play" ? "native-state-playback" : "observed-runtime",materialObservation));
            }
            else throw new ArgumentException("Unknown op; use inventory/reload/capture/play/select-room/stage-enemy/fortify-party/return-to-title/quiet-tutorials/lease-test.");
        }
        catch (Exception ex)
        {
    ...
    JObject Snapshot(SkinnedMeshRenderer renderer, bool full)
    {
        ...
        JObject result = new JObject {
            ...
            {"celInstanceId",cel==null?0:cel.GetInstanceID()}, {"celRootName",cel==null?null:cel.name}, {"celRelativeRendererPath",celPath},
            {"enemyRelativeRendererPath",enemy==null?null:Relative(renderer.transform,enemy.transform)},
            {"ownerRelativeRendererPath",OwnerPath(renderer,avatar)},
    ...
    IEnumerator Capture(string id,SkinnedMeshRenderer renderer,float seconds,float fps,int maxWidth,bool fixedStep,string provenance,bool materialObservation=false,bool arrivalObservation=false)
    {
        ...
        try
        {
            ...
            for(int index=0;index<count;index++)
            {
                ...
                try
                {
                    ...
                    JObject pose=Snapshot(renderer,false);if(materialObservation)pose["materialObservation"]=MaterialObservation(renderer,index==0,"end-of-frame-before-png-readback");pose["frame"]=Time.frameCount;
    ...
    JObject FortifyParty(JObject command)
    {
        ...
        List<CharacterOverworld> heroes=new List<CharacterOverworld>();List<CharacterStats> statsList=new List<CharacterStats>();JArray before=new JArray();
        foreach(object item in party)
        {
            ...
            heroes.Add(cow);statsList.Add(stats);before.Add(HeroHealth(stats,cow));
        }
        ...
        for(int i=0;i<statsList.Count;i++)
        {
            ...
            after.Add(HeroHealth(stats,heroes[i]));
        }
        fortifiedMaxHpTarget=target;
        return new JObject{{"ok",true},{"provenance","boosted-HP disposable test fixture; combat actions remain native, balance is not representative"},
            {"targetMaxHp",target},{"nativeReadyFixture",readyFixture},{"before",before},{"after",after}};
    }
    ...
    void OnDestroy(){enemyLifetime=null;if(spawnCaptureObserver==this)spawnCaptureObserver=null;if(entryTicket!=null)entryTicket.valid=false;krakenSkinArm=null;portraitArmed=false;portraitTrace.Clear();if(portraitObserver==this)portraitObserver=null;RestoreTutorials();watchedLeases.Clear();}
    ...
    {
        string enemy=Str(command,"enemy");
        ...
        MiniHexDungeon.RoomInfo replacement=new MiniHexDungeon.RoomInfo(MiniHexDungeon.EncounterType.Enemy,null,new[]{verifiedId},-1);
        ...
        return new JObject{{"ok",true},{"enemy",verifiedId},{"level",level},{"room",room},{"regenerated",regenerate},
