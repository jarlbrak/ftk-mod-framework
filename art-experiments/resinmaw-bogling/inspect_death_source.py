...
r=f.objects[121344];g=ref(r,tree(r)['m_GameObject']);t=next(ref(g,c['component'])for c in tree(g)['m_Component']if ref(g,c['component']).type.name=='Transform')
...
for o in f.objects.values():
 ...
 d=tree(o)
 if 'acid' not in d.get('m_Name','').lower() or 'death' not in d['m_Name'].lower():continue
