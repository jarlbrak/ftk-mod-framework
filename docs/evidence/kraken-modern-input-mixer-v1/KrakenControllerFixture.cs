    ...
    IEnumerator KrakenControllerFixture(string id,JObject command)
    {
        ...
        try
        {
            try
            {
                ...
                {
                    if(command["modernInputMixer"].Type!=JTokenType.String || (string)command["modernInputMixer"]!=KrakenModernInputMixer.Mode || command["endpointPolicy"]!=null || command["observeExistingDrivers"]!=null || krakenSkinArm!=null)
                ...
                if(modernMixer!=null)modernMixer.Create(modernSource,native,id);
