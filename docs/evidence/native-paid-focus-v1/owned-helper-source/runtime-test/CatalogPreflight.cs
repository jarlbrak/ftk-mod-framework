...
public sealed partial class RuntimeModelTest
{
    ...
    {
        CatalogKeys(profile,"key","baseEnemy","displayName","combatProfile","renderers","resourcePrefab","minimumBaseHealth","visualScale","portraitMarkerPath");
    ...
    JObject CatalogRenderers(CharacterEventListener cel,JArray assignments,JArray apparel,MethodInfo load,MethodInfo resolve,bool enemyOptions=false)
    {
        ...
        foreach(JToken token in combined)
        {
            ...
            try
            {
                ...
                CatalogKeys(assignment,optional?new[]{"rendererPath","glbFile","textureFile","expectedNativeMeshName"}:enemyOptions?new[]{"rendererPath","glbFile","textureFile","disableNativeEmission","materialSlots"}:new[]{"rendererPath","glbFile","textureFile"});
                ...
                string path=Str(assignment,"rendererPath"),name=Str(assignment,"glbFile");result["celRelativeRendererPath"]=path;result["glbFile"]=name;
                ...
                SkinnedMeshRenderer renderer=matches[0];Mesh original=renderer.sharedMesh;
                ...
                result["nativeMesh"]=original.name;result["rendererInstanceId"]=renderer.GetInstanceID();result["boneSignature"]=BoneSignature(renderer);
                result["bones"]=renderer.bones.Length;result["nativeBindposes"]=original.bindposes.Length;
                int nonnull=0;foreach(Material material in renderer.sharedMaterials)if(material!=null)nonnull++;
                result["materialSlots"]=renderer.sharedMaterials.Length;result["nonnullMaterialSlots"]=nonnull;
                if(materialSlots==null)decoded=load.Invoke(null,new object[]{name,renderer.bones,original.bindposes,true})as Mesh;
                else
                {
                    if(renderer.sharedMaterials.Length!=materialSlots.Length || nonnull!=materialSlots.Length)throw new InvalidOperationException("Multi-slot mode must preserve all nonnull native slots.");
                    Material[] nativeMaterials=renderer.sharedMaterials;JArray nativeSlots=new JArray();
                    ...
                    decoded=multi.Invoke(null,new object[]{name,renderer.bones,original.bindposes,true,mapping})as Mesh;
                    Material[] afterMaterials=renderer.sharedMaterials;if(afterMaterials.Length!=nativeMaterials.Length)throw new InvalidOperationException("Native material array changed during decode preflight.");
                ...
                result["vertices"]=decoded.vertexCount;result["triangles"]=decoded.triangles.Length/3;result["decodedBindposes"]=decoded.bindposes.Length;
                if(renderer.sharedMesh!=original)throw new InvalidOperationException("Unexpected native mesh identity change.");
