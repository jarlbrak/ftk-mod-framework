...
import numpy as np
OUT=Path(__file__).resolve().parent;ROOT=OUT.parent.parent;target=ROOT/'scratch/bronzehollow-original-proof';target.mkdir(exist_ok=True)
...
files=[]
for name in ['bronzehollow.source.json','bronzehollow_basecolor.png','bronzehollow.pieces.json']:
