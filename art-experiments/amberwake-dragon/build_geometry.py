...
for name in ['Root_M','BackA_M','BackB_M','Chest_M','Neck_M','NeckPart1_M','Tail1_M','Tail2_M','Tail3_M','Tail4_M']:
 c=b.B[name];b.tube('Amber dorsal crest '+name,[c+[0,.33,0],c+[0,.63,-.08],c+[0,.70,-.35]],[.15,.10,.01],[.22,.16,.01],[name]*3,5,sides=4)
for side in ['R','L']:
 sg=1 if side=='R' else -1
 ns=['Scapula_'+side,'frontHip_'+side,'frontKnee_'+side,'frontAnkle_'+side,'frontBall_'+side]
 b.tube('Front shoulder and leg '+side,[b.B[n] for n in ns],[.43,.38,.25,.19,.21],[.38,.33,.23,.18,.10],ns,[0,1,2,1,0,1,3,3],axis=(1,0,0))
 ...
 for k,end in enumerate(ends,1):
  ...
  b.tube('Wing wrist connector '+side+str(k),[b.B[side+'_Wing_3'],b.B[names[0]]],[.16,.15],[.14,.13],[side+'_Wing_3',names[0]],1,axis=(0,1,0),sides=6)
 ...
 names=[side+'_Wing_2',side+'_Wing_3',chains[2][0],chains[2][1]]
 panel('Proximal wing membrane '+side,[b.B[n] for n in names],names,4)
 names=[side+'_Wing_3',chains[0][0],chains[1][0],chains[2][0]]
 panel('Wrist wing membrane '+side,[b.B[n] for n in names],names,5)
